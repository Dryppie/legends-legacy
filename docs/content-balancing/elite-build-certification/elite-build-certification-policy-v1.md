...
| Scenario-niche quality-diversity island | Disabled (`0`) | Disabled (`0`) |
| Mechanic-archetype quality-diversity island | Disabled (`0`) | Disabled (`0`) |
| Valley beam and metadata prefilter | Disabled | Disabled |
| Restart bridge audit | Disabled | Disabled |
| E5 descriptor-separability audit | Disabled | Disabled |
...
`--elite-quality-island` is a separate ceiling-preserving quality-diversity experiment. After the complete restart-local baseline optimizer and refinement finish, it pre-fills a behavioral map from that restart's own authoritative baseline population, evaluates a 32-candidate deterministic legal seed batch, and spends the rest of its fixed budget on one-substitution descendants of current niche champions. The implemented niche is the strongest/weakest production PvE benchmark scenario pair. Every island candidate is authoritatively benchmarked; another restart's candidates, bridge nodes, and descendants cannot become parents. Reports serialize initial and descendant evaluations, final niche occupancy, champion replacements, island ceiling/build/genome, and baseline/final ceilings. The final result must dominate its baseline. Island mode is mutually exclusive with crossover, coordinated mutation/archive, stratified portfolio, and valley search, and is disabled in both approved profiles.

`--elite-mechanic-island` runs the same isolated, ceiling-preserving island protocol with the audit-validated eight-axis mechanic-archetype niche instead of scenario pairs. The eight binary authored-behavior axes impose a hard ceiling of 256 niches. Reports keep its evaluations, occupancy, replacements, ceiling/build/genome, and E5 high-niche collision telemetry separate from the scenario island. The high-niche comparison is report-only and cannot affect candidate generation, parent selection, archive replacement, scoring, refinement, or verdicts. Mechanic-island mode cannot be combined with crossover, coordinated mutation/archive, stratified portfolio, scenario-island, or valley search, and is disabled in both approved profiles.
...
`--elite-descriptor-audit` is a separate opt-in E5 topology diagnostic. It authoritatively benchmarks the three known high/low anchors and every unique legal one-substitution neighbor, compares the optimizer-retained baseline set, and measures source-family, resolved authored mechanic, generic eight-axis mechanic-archetype, resolved authored effect-role, and centered scenario-shape descriptors. It also audits a four-axis capped mechanic-intensity residual across finalized independently generated E5 candidates in the high coarse niche. Scores define high/low audit outcomes but never descriptor features; leave-one-out exact-signature classification prevents a candidate from voting for itself. Reports include purity, fragmentation, high/low/balanced accuracy, collisions, exact high-niche occupancy, hard ceilings, map readiness, feature contrasts, basin score ranges, and scenario centroids. Audit candidates and audit-derived models are never added to certification populations, refinement, percentile cohorts, finalists, ceilings, verdicts, or `TotalUniqueCandidatesEvaluated`; the option is disabled in both approved profiles.

`--elite-benchmark-confidence-audit` is a separate opt-in E5 sampling diagnostic. It selects a deterministic score-stratified cohort while forcing known anchors and certification finalists into the sample, then repeats the five production PvE scenarios with common scenario seeds. The audit reports approximate 95% score half-widths, recommended seed counts for a configured margin, baseline/replicate rank correlations, top-k overlap, and paired anchor intervals. Its executions and results are audit-only and cannot affect search, certification evidence, or verdicts. The option is disabled in both approved profiles.
...
Algorithm v15 tested a `256`-candidate restart-local quality-diversity island per profile. The corrected scenario-pair run benchmarked `2,304` island candidates, retained `62,865` unique certification candidates, and completed in approximately `256` seconds. Islands occupied 7–11 niches and made 1–13 champion replacements, but no island exceeded its refined baseline. E4 failed at `0.63` (`77.98`, `78.61`, `78.32`), E5 failed at `1.30` (`86.21`, `84.91`, `85.27`), and E6 retained a passing `0.34` baseline spread. The result remained `SearchUnstable`; seed `1337` was not run and no setting was promoted. The next step is an audit-only descriptor-separability study, not a larger island budget.

Algorithm v16 completed that audit on seed `8471`. It evaluated `1,112` audit-only E5 genomes in complete distance-one neighborhoods while leaving `60,903` certification candidates and all restart outcomes unchanged. Source-family and effect-role descriptors classified the neighborhoods but failed because `94.06%` and `100%` of candidates, respectively, occupied singleton signatures. Centered scenario shape failed at `47.43%` balanced nearest-anchor accuracy and `55.31%` singleton rate. Resolved authored mechanics passed the family-level criteria with `96.76%` balanced accuracy, `99.28%` purity, and `26.17%` singleton rate, but its full count signature still produced `472` niches and no optimizer-retained candidate occupied the exact high-anchor signature. This justifies auditing a generic coarse mechanic descriptor; it does not justify a larger scenario island or direct use of the full mechanic vector.

Algorithm v17 validated the prospective `mechanic-archetype` descriptor without changing the authoritative candidate set or certification results. Eight generic binary axes hard-bound the map to `256` possible niches. Across the same complete distance-one E5 neighborhoods it produced `55` observed signatures, `97.57%` purity, `0.63%` singleton candidates, and `88.43%` balanced nearest-anchor accuracy, with no anchor collision. It is the only audited descriptor that passes both separation and an explicit hard niche ceiling. The next permitted experiment is therefore one small `256`-candidate restart-local mechanic-archetype island on seed `8471`, with complete baseline preservation and no audit/cross-restart candidate input.

Algorithm v18 completed that experiment. The seed-`8471` run benchmarked `2,304` restart-local mechanic-island candidates, retained `63,023` unique certification candidates, and completed in approximately `303` seconds. Islands occupied 133–153 niches and made 28–44 champion replacements, but no island beat a refined baseline. E4 failed at `0.63`, E5 failed at `1.30` (`86.21`, `84.91`, `85.27`), and E6 retained its passing `0.34` spread. The known high E5 coarse niche was present in every complete baseline and was also reached by every island, yet same-niche scores ranged from the known `86.21` down into the 70s. This establishes descriptor collision rather than insufficient coarse-niche coverage. The result remained `SearchUnstable`; seed `1337` was not run, the option stays disabled, and no budget is promoted. Any next step must first be an audit-only, hard-bounded residual mechanic-descriptor study inside the colliding coarse niche.

Algorithm v19 completed that audit-only follow-up. The corrected seed-`8471` run examined 343 finalized independently generated E5 candidates in the coarse high niche: one scored at least `85.71`, 342 scored at or below `85.27`, and none fell in the excluded gap. The four capped `0/1/2+` authored-mechanic intensity axes produced three signatures under the hard 81-niche ceiling. Purity was `99.71%` only because of the 1:342 class imbalance; high accuracy was `0%`, low accuracy `99.71%`, balanced accuracy `49.85%`, and the high anchor's residual signature collided with low candidates. The descriptor is not map-ready. Seed `1337` was not run and no setting is promoted. Further work must first determine, through audit-only held-out ranking, whether quantitative effect payloads or higher-order pair interactions contain a prospective signal.
...
6. Test one `256`-candidate restart-local, ceiling-preserving mechanic-archetype island on seed `8471`; do not encode anchor Essence identities or share audit/cross-restart candidates.
7. If seed `8471` passes without losing a known ceiling, repeat the candidate configuration on seed `1337`.
8. Stabilize independent generic search on both root seeds without weakening the frozen tolerances or losing the observed ceiling.
9. Run the complete release profile with reviewed curated evidence.
10. Permit `CertifiedElite` only when every required evidence source passes.
