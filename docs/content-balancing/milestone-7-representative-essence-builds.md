...
Balance schema version 7 introduced `representativeBuilds` in `summary.json` and `representative-builds.json` under both `latest` and immutable history; the current combined pipeline uses schema version 46. `summary.md` reports every profile's target score, selected score range, mean score, mean pairwise similarity, and representative count, followed by a compact list of its closest representative.
