...
**Status: Authored-content progression certification, optional balance-only scaling probes, and their performance baseline are implemented in schema 23; release evidence is not yet approved.** Schema 23 retains build-capability profiler algorithm v2, party-family builder algorithm v2, and authoritative party-family encounter evaluation algorithm v3. The party-family artifact keeps the explicit Region 1 family response shapes at authored roster sizes and adds balanced P50/P75/P90 progression cohorts. P75 reuses the intended-balanced family execution, while P50 and P90 are independently constructed from their capability-profiled representative libraries and run unchanged against authored production World Tower content. The certification policy requires complete progression evidence in addition to three constraint-passing regular rosters per family/cohort, 25 common-seed trials per roster, 100 optimized holdout trials, maximum 0.25 confidence-interval width, typed mechanic evidence, and certified release-profile elite evidence. Expected P50 ≤ P75 ≤ P90 ordering has a frozen five-percentage-point tolerance: overlapping point inversions produce `ReviewRequired`, while confidence-separated inversions produce `Failed`. Developer CLI runs remain non-certifying at one trial per roster; release runs default to 25. Direct runner calls remain opt-in to avoid silently multiplying diagnostic workloads. Production content retains each floor's authored `RequiredSlots`. The opt-in `--scale-probes` path constructs balanced-P75 5/10/15 rosters, clones the floor definition in memory, applies the existing participant formulas and any explicit diagnostic content multipliers, and runs the production World Tower combat executor. Its separate immutable artifact reports Wilson confidence, authored-size deltas, failure observations, formula ratios, exact added trial/tick workload, wall time, current-thread allocations, trial/tick throughput, process peak working set, managed-heap high-water estimates, and runtime/host context. The four-run [workstation performance baseline v1](encounter-scale-probe-performance-baseline-v1.md) freezes a repeatable 450-trial workload and opt-in diagnostic thresholds; its validation run passed every floor/size batch. The artifact is always non-release and never creates a gameplay variant. See [Elite Build Certification Gate](elite-build-certification/elite-build-certification-gate.md) for the elite search and curated-player dependency. The checked-in player fixture remains empty, so no current Region 1 run can satisfy that dependency. An actual release-profile run remains before Region 1 remediation is accepted.

Balance schema 24 also starts Phase 5 assisted encounter calibration. The new opt-in path keeps the shared health/offense search as its diagnostic baseline, gates one-parameter sensitivity probes from dominant observed failure evidence, performs paired independent holdout revalidation, and emits bounded human-review proposals or `Review`. It currently supports offense for primary-target/party-attrition observations and regeneration for boss-sustain observations; ambiguous mechanic evidence never selects a knob. See [Assisted Encounter Calibration](assisted-encounter-calibration.md).
...
Developer stabilization is currently blocked by E5 search agreement on seed `8471`: one restart remains at `85.27`, four substitutions from the `86.21` optimum reached by the other two restarts, producing a `0.94` spread against the frozen `0.50` tolerance. The exhaustive width-16/depth-3 valley diagnostic took `1,538.39` seconds and did not close the gap. A metadata-prefiltered follow-up took `1,249.75` seconds, still missed E5, and pruned away a stronger E4 ceiling despite reporting zero E4 spread. Both settings remain disabled.

Algorithm v11 completed the isolated shortest-path bridge audit with the same `96` population, `24`-`40` generation, and `12` elite settings, with crossover and valley search disabled. The run took `1,247.34` seconds, retained `59,006` certification candidates, and separately evaluated `146` bridge nodes. All 70 legal E5 minimum-substitution nodes were benchmarked. The best maximin path fell from `85.27` to `83.69` before reaching `86.21`, required a `1.30` single-step regression, accumulated `4.28` points of temporary deficit, and had neither a non-regressing nor frozen-`0.50`-bounded route. E5 is therefore separated by a genuine shortest-path fitness valley, though longer detours were not tested. Certification evidence and verdicts remained unchanged.

Algorithm v12 added an opt-in restart-local coordinated-mutation rate that replaces ordinary births with exact legal three/four-gene jumps and reports their counts separately. A `20%` seed-`8471` trial made `3,366` such jumps within the existing population and maximum-generation budgets, took `1,407.29` seconds, and evaluated `62,296` certification candidates. It recovered the stronger E4 `78.61` ceiling at `0.46` spread, but E5 worsened to `1.09` spread (`86.21`, `85.50`, `85.12`) and E6 widened to `0.37`. The setting failed convergence and runtime, so seed `1337` was not repeated and no budget or default is promoted. The next bounded investigation is a persistent restart-local exploratory lineage or archive that can carry temporarily weaker descendants across generations without altering the official elite or importing cross-restart knowledge.

Algorithm v13 tested a 12-candidate persistent archive at the same `20%` rate. It produced `1,664` coordinated seeds and `1,513` continuation births, took `1,386.22` seconds, and evaluated `59,793` candidates. E5's spread fell to `0.15`, but only because all restarts lost the `86.21` ceiling and converged on `85.27`/`85.12`; E6 widened to `0.99`, and E4 retained only `78.32`. The overall result remained `SearchUnstable`, runtime failed, and seed `1337` was not repeated. Persistent undirected exploration is therefore also rejected. The next bounded investigation should preserve and isolate the baseline candidate stream while testing deterministic stratified coverage or a portfolio strategy; no budget or default is promoted.

Algorithm v14 implemented the isolated portfolio follow-up with `256` deterministic candidates per restart/profile. Baseline optimization and refinement complete first; portfolio refinement is separate, and the report serializes both ceilings. The seed-`8471` run directly benchmarked `2,304` portfolio genomes, evaluated `80,560` local candidates, retained `109,724` unique certification candidates, and completed in approximately `517.35` seconds. It preserved the known E4/E5/E6 ceilings. E4 passed at `0.29` spread and E6 at `0.34`, but E5 remained unstable at `1.09` (`86.21`, `85.12`, `85.27`). Seed `1337` was not run, the frozen `0.50` tolerance is unchanged, and neither the portfolio nor its expanded refinement work is promoted. If investigation continues, the next bounded strategy should be an isolated quality-diversity island with explicit mechanic/source-family niches rather than a larger undirected portfolio.

Algorithm v15 implemented that isolated quality-diversity follow-up with a `256`-candidate budget per restart/profile. The corrected seed-`8471` run pre-filled strongest/weakest-production-scenario niches from each restart's complete authoritative baseline, evaluated 32 fresh candidates and 224 one-swap niche-champion descendants, retained `62,865` unique certification candidates, and completed in approximately `256` seconds. Islands occupied 7–11 niches and replaced champions 1–13 times, but none beat a refined baseline. E4 failed at `0.63`, E5 failed at `1.30` (`86.21`, `84.91`, `85.27`), and E6 retained its passing `0.34` baseline spread. Seed `1337` was not run; the option stays disabled and no budget or tolerance is promoted. The next bounded investigation is audit-only descriptor separability for the known E5 basins before another search run.

Algorithm v16 completed that audit behind `--elite-descriptor-audit`. The seed-`8471` run authoritatively evaluated `1,112` audit-only E5 anchor and complete distance-one neighborhood genomes while retaining `60,903` independent certification candidates and unchanged `0.63`/`1.30`/`0.34` E4/E5/E6 spreads. Source-family and effect-role signatures were nearly or completely singleton-fragmented, while centered scenario shape reached only `47.43%` balanced nearest-anchor accuracy. Resolved authored mechanic features reached `96.76%` balanced accuracy and `99.28%` purity with `26.17%` singleton candidates, demonstrating a real mechanic signature. The full mechanic vector still formed `472` niches and had no exact high-anchor match in the 17 optimizer-retained E5 candidates. No search setting is promoted. The next bounded investigation is a generic coarse mechanic descriptor with a hard niche ceiling, validated before any small restart-local mechanic-island trial.

Algorithm v17 added that prospective coarse descriptor to the unchanged audit. Eight anchor-independent binary mechanic axes impose a theoretical maximum of `256` niches. The same `1,112` E5 neighborhoods produced only `55` signatures, with `97.57%` purity, `0.63%` singleton candidates, and `88.43%` balanced nearest-anchor accuracy. The artifact marks only `mechanic-archetype` as map-ready. Search evidence remained unchanged and no setting was promoted. The next bounded experiment is one `256`-candidate restart-local mechanic-archetype island on seed `8471`, with authoritative benchmarking, full baseline dominance, and no audit or cross-restart candidate input.

Algorithm v18 completed that bounded mechanic-archetype island experiment. The seed-`8471` run benchmarked `2,304` island candidates, retained `63,023` unique certification candidates, and completed in approximately `303` seconds. It occupied 133–153 niches per restart/profile and recorded 28–44 champion replacements, but no island beat its fully refined baseline. E4/E5/E6 spreads remained `0.63`/`1.30`/`0.34`. The known high E5 coarse niche was present in every complete baseline and reached by every island, yet same-niche scores ranged from `86.21` into the 70s. This rejects the eight-axis key as a sufficiently precise fitness-basin map. Seed `1337` was not run, the option remains disabled, and no budget or tolerance is promoted. Further work should first be an audit-only collision study for a small hard-bounded residual authored-mechanic descriptor; no larger island search is justified.

Algorithm v19 completed the collision study by adding a four-axis capped `0/1/2+` mechanic-intensity residual with a hard 81-signature ceiling. The corrected seed-`8471` audit used all finalized independently generated E5 certification candidates in the parent coarse niche and kept audit neighborhoods outside that population. It found 343 candidates: one at or above the `85.71` high floor and 342 at or below the `85.27` low ceiling. The residual formed only three signatures; its `99.71%` purity reflected class imbalance, while high accuracy was `0%`, balanced leave-one-out accuracy was `49.85%`, and the high residual collided with low candidates. It is not map-ready. Search evidence and the `0.63`/`1.30`/`0.34` spreads were unchanged, seed `1337` was not run, and no setting is promoted. Further work should first be an audit-only leave-one-restart-out rankability study of quantitative effect payloads and higher-order Essence interactions; another island search is not justified.

Algorithm v20 first validated whether current sampling could support that model study. A 512-build E5 cohort repeated over 16 common seeds and five scenarios failed the predeclared ranking, top-20 overlap, and `0.25` score-margin gates; the observed maximum variance implied up to 446 seeds for that precision. A separate balanced 80-Essence singleton round robin ran 50,560 battles and achieved exactly 1,264 battles per Essence, but all 80 scores were `0.5000`. The meta analyzer now rejects this as `SimulatorNoDiscrimination` instead of labeling every Essence healthy. Pair-interaction modeling is blocked until a neutral singleton endpoint demonstrates non-zero discrimination.

Algorithm v21 then tested practical whole-build seed panels against a 32-seed common-random-number reference. The 612-build cross-profile audit executed 97,920 authoritative scenarios in 48.8 seconds. No submaximal panel passed the predeclared top-k, elite/finalist pair-order, separated-reversal, and consecutive-panel gates. Four seeds remained within the projected 15-minute search target but were not reliable in the elite region; 24 seeds were much more stable but projected to 60.8 minutes. The reference itself projected to 80.9 minutes. The robust objective and population-size search experiment are not authorized. A cached progressive-evaluation design is the next bounded measurement experiment.
...
Schema-46 JSON and Markdown reporting, including optional non-release 5/10/15 scale probes, reliability fault injection, matched-genome progression diagnostics, upstream population-protocol provenance, and host-dependent performance evidence
```
...
The Region 1 reliability-validation milestone is complete as an audit, but its expansion gate is failed. Milestone 14 is deferred rather than the next active implementation task. Before beginning it, explicitly resolve or approve all Region 1 blockers:

The [Region 1 affected-family contract decision](region-one-family-contract-decision.md) is the author-review packet for the unresolved Regeneration and DistributedAttrition premises. It proposes absolute physical contract shapes without inventing numeric limits or promoting a family verdict.

```text
Supply reviewed top-player fixtures and pass the Elite Build Certification Gate in release mode
Author Regeneration and DistributedAttrition affected-family contracts, or explicitly accept that their family identification remains insufficient
Add a real measurable player cleanse capability and valid specialist rosters before enabling CleanseDemand validation
Retain the smooth-step progression unless an author-owned neutral-reference/cohort policy is preregistered and passes a fresh schema-46 protocol-matched panel
...
Until those author/content prerequisites are resolved, the approved action is to retain the current Region 1 smooth-step progression and make no Region 2+ expansion. Full equipment optimization, distributed simulation, live telemetry, automatic production-data mutation, and expansion beyond the proven Region 1 band remain outside the current implementation boundary.
