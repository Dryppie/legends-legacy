...
Balance schema version 12 introduced `encounterCalibration` in `summary.json` and `encounter-calibration.json` under both `latest` and immutable history; the current combined pipeline uses schema version 14. Each floor records:
