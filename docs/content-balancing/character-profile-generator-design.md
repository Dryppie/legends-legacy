...
- Context-aware legacy-family selection remains available outside the exact-floor Tower portfolio. Generator 23 uses floor-specific `CalibrationTeam` selection for Tower and treats PvP discovery score only as a deterministic secondary signal after production Tower evidence.
...
- Explicit, configurable legacy population buckets remain available for non-Tower profile portfolios. Exact-floor World Tower generation instead emits four equally weighted `CalibrationTeam` expeditions plus a zero-weight No-Essence diagnostic control.
...
- Profile schema/generator version 7/23 includes explicit floor-specific scenario identity and coverage, party composition and evidence, production Tower context qualification, separate matchup safeguards, and bounded direct Tower candidate search. Team/profile identities include the exact floor, content, roster size, equipment tier/rarity/quality, source-audit equipment profile, and Essence count, so adjacent progression or guardian contexts cannot collide.
...
- A diagnostic World Tower certification runner now uses one explicit, fixed seed manifest shared across canonical below/recommended/stronger cohorts, every profile team, campaign runs, and 100-sample anchor confirmation. Campaign identity is provenance rather than a hidden source of new random samples. Certification requires complete exact-size profile coverage, an exact selected-scenario match to the production-prepared recommended equipment and Essence context, configurable minimum samples, monotonic canonical outcomes, every selected exact-context team to remain strictly below a 20% estimated win rate, and at least one such team to be strictly above 5%, with bounded timeout rate and valid production-runtime evidence. A team at exactly 5% does not provide the required anchor; a team at exactly 20% fails the floor. Per-team confidence intervals, weighted population outcomes, and equal-context spread remain diagnostics.
- Expanded World Tower generation qualifies discovery finalists and up to 500 additional legal parties directly against one exact floor. Generator 23 emits four `CalibrationTeam` expeditions plus one No-Essence control per floor. Every emitted team is confirmed with 100 production samples and must be strictly below 20%; at least one CalibrationTeam must be strictly above 5% and below 20%. Historical PvP family labels are not attached to teams that fail the Tower contract. Floor-specific scenario IDs prevent mechanically different guardians with identical equipment requirements from being forced to share one portfolio.
...
- Generator-13 campaign `66368b83-07c1-4a7a-baf6-487c65fc8492` reused the five audits, generated 13 valid sets, and passed smoke plus the obsolete contract-3 certification interpretation. Reassessment under corrected contract 4 rejects all 15 floors because each includes at least one team at or above 20%; floor 1 also lacks a strict `>5%` and `<20%` anchor. The candidate is not eligible for promotion, and the approved source catalog is unchanged.
- Generator-23 campaign `c27ad5ef-8483-4b30-b413-62e92f0443a1` reused the five discovery audits and the freshly materialized 15-set floor-specific catalog, passed production smoke, and passed contract-4 100-sample certification with zero issues. It contains 75 teams; every team is below 20%, every floor has at least one strict anchor, and the largest observed team estimate is 19% on floor 11. The candidate is promotion-ready but has not been written to the source-controlled approved catalog.
...
- Review and source-control promotion of generator-23 campaign `c27ad5ef-8483-4b30-b413-62e92f0443a1`. The committed catalog deliberately remains empty until that explicit review is complete.
- Approved World Tower portfolios for every required scenario. Until a corrected-contract candidate is reviewed and committed, the approved-catalog shadow report will deliberately show coverage gaps instead of extrapolating smaller or differently equipped teams.
