...
- Profile schema/generator version 7 includes explicit scenario identity and floor coverage, party composition and evidence, production Tower context qualification, separate matchup safeguards, and directional adversary confidence. Team/profile identities include the exact content, roster size, equipment tier/rarity/quality, source-audit equipment profile, and Essence count, so adjacent progression contexts cannot collide.
...
- A diagnostic World Tower certification runner now uses an explicit seed manifest shared across canonical below/recommended/stronger cohorts and every profile team. It requires complete exact-size profile coverage, an exact selected-scenario match to the production-prepared recommended equipment and Essence context, configurable minimum samples, Wilson 95% confidence intervals inside the authored success bands, monotonic canonical outcomes, bounded equal-context profile spread, and bounded timeout rates.
...
- A fresh fingerprint-contract-3 World Tower campaign using role-aware discovery and production finalist qualification. Earlier completed campaigns cannot be promoted or safely retried as current evidence.
