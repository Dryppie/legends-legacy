...
| Guild donation | Player donates resources. | `GuildController.Donate` -> `DonateToGuildCommand` -> `IGuildService.DonateToGuildAsync`. | `GuildStateService.donate()` refreshes authoritative guild snapshot after success. | REST boolean plus snapshot refresh; SignalR for other guild members. | Safe mitigation implemented; compact donation result DTO remains optional future cleanup. |
