...
| SignalR hub | `GameHub : Hub<IGameClient>` adds connections to a `char:{CharacterId}` group in `OnConnectedAsync`; supports `SubscribeToAudience(AudienceDto)` for world and a commented guild path. | `LL/src/Infrastructure/RealTime/RealTime.LL/GameHub.cs` | Hub is thin, which is good. There is no `[Authorize]` attribute or `.RequireAuthorization()` on `MapHub<GameHub>("/hub")` in the inspected code. It relies on a valid `CharacterId` claim and throws a `HubException` if missing. |
...
| SignalR envelope | `GameEventEnvelope` contains `Event` and `Payload`. | `LL/src/Infrastructure/RealTime/RealTime.LL/GameEventEvelope.cs` | File name is misspelled as `GameEventEvelope.cs`. Envelope has no `OccurredAt`, `Sequence`, version, source, or idempotency metadata. |
| Realtime publisher | `GameEventPublisher` sends `GameEventEnvelope` to character, guild, or world audiences using SignalR groups. | `LL/src/Infrastructure/RealTime/RealTime.LL/GameEventPublisher.cs`, `LL/src/Core/Application/Interfaces/WebSockets/IGameEventPublisher.cs`, `LL/src/Core/Application/Interfaces/WebSockets/Audience.cs` | `Audience.Guild` publisher exists, but hub-side guild subscription authorization/joining is commented out. |
| Realtime DTOs | `GameEventMsg` includes `SaleCompletedMsg`, `GuildApplicationMsg`, `GuildBuildingUpgradedMsg`, `RiftOpenedMsg`; `LootReceivedMsg` is separate and active. | `LL/src/Core/Application/WebSockets/Contracts/GameEventMsg.cs`, `LL/src/Core/Application/WebSockets/Contracts/LootReceivedMsg.cs` | Only `LootReceivedMsg` has confirmed frontend handling. Other records are defined but not wired in inspected frontend state. |
...
| Dungeon reward/dismiss commands | `ClaimDungeonRewards` and `DismissFailedDungeonRun` return `Response<bool>`. | `LL/src/Core/Application/UseCases/Dungeons/Commands/ClaimDungeonRewards/ClaimDungeonRewardsCommand.cs`, `LL/src/Core/Application/UseCases/Dungeons/Commands/DismissFailedDungeonRun/DismissFailedDungeonRunCommand.cs` | Frontend clears active dungeon locally after success; response does not include updated inventory, character resources, or replacement active-run state. |
| Colosseum command | `StartArenaBattle` returns `Response<CombatResultDto>`. Service spends an arena ticket before running combat. | `LL/src/API/API.LL/Controllers/V1/ColosseumController.cs`, `LL/src/Core/Application/UseCases/Colosseum/Commands/StartArenaBattle/StartArenaBattleCommand.cs`, `LL/src/Infrastructure/Service/Services.LL/Colosseum/ColosseumService.cs` | The ticket decrement is not returned to the caller. This is the clearest mismatch with the target rule. |
...
| Colosseum internal side effects | `ArenaBattleCompletedEventHandler` calculates ratings and saves match result. | `LL/src/Core/Application/UseCases/Colosseum/EventHandlers/ArenaBattleCompletedEventHandler.cs` | The REST response does not include updated rating or match result. The event is internal and does not currently push SignalR. |
...
| Character action commands | Start/stop action endpoints return `Response<bool>`. | `LL/src/API/API.LL/Controllers/V1/CharacterActionsController.cs`, `LL/src/Core/Application/UseCases/CharacterActions/Commands/StartCombatAction/StartCombatActionCommand.cs` | Frontend starts polling after a successful boolean rather than applying a returned `CharacterActionDto`. |
| Market commands | Create listing returns `MarketPlaceListingDto`; buyout and cancel return `Response<bool>`. | `LL/src/API/API.LL/Controllers/V1/MarketPlaceController.cs`, `LL/src/Core/Application/UseCases/MarketPlaces/Commands/BuyoutMarketPlaceListing/BuyoutMarketPlaceListingCommand.cs`, `LL/src/Infrastructure/Service/Services.LL/MarketPlaces/MarketPlaceService.cs` | Buyout mutates buyer cinders, seller cinders, buyer inventory, and listing quantity, but response is only `bool`. Seller notification is a natural SignalR candidate. |
| Equipment commands | Equip/unequip endpoints return `Response<bool>`. | `LL/src/API/API.LL/Controllers/V1/EquipmentController.cs`, `LL/src/Core/Application/UseCases/Equipments/Commands/EquipEquipment/EquipEquipmentCommand.cs`, `LL/src/Core/Application/UseCases/Equipments/Commands/UnequipEquipment/UnequipEquipmentCommand.cs` | Caller needs updated equipment slots, inventory, and derived character stats. |
| Essence commands | Some commands return result DTOs, but absorb/ascend/evolve/favorite/activate/delete return message-only `ResponseMessageDto`; frontend refreshes archive/loadouts after most operations. | `LL/src/API/API.LL/Controllers/V1/EssenceController.cs`, `LL/src/Presentation/ll/src/app/core/services/api/essences/essence-state.service.ts` | Good snapshot APIs exist: archive and loadouts. Command responses should progressively include affected archive/loadout/inventory/stat state. |
...
| Central game SignalR client | `GameEventService` owns one `HubConnection`, connects to `${environment.apiBaseUrl}/hub`, uses cookies with `withCredentials: true`, registers `Publish`, and exposes one signal per event through a `Proxy`. | `LL/src/Presentation/ll/src/app/core/services/real-time/game-event.service.ts` | This is a good foundation, but no connection-status signal, reconnect callbacks, resync hooks, sequence handling, or error state were found. |
| Realtime lifecycle facade | `RealTimeFacade` connects when `AuthService.isAuthenticated()` is true and disconnects otherwise. It is initialized through `APP_INITIALIZER`. | `LL/src/Presentation/ll/src/app/core/services/real-time/real-time-facade.ts`, `LL/src/Presentation/ll/src/app/app.config.ts` | Facade currently connects with `{ kind: 'World' }`, but `GameEventService` has the hub `SubscribeToAudience` invocation commented out. Character group join is automatic on the server. |
| Game realtime event map | Frontend event map contains only `LootReceivedMsg`. | `LL/src/Presentation/ll/src/app/core/services/real-time/game-event/game-event.map.ts`, `LL/src/Presentation/ll/src/app/core/services/real-time/loot/loot-received.ts` | Other backend websocket records are not typed in the frontend registry. |
| Inventory state | Signal-based `InventoryStateService` loads inventory snapshot, applies `LootReceivedMsg`, and has manual mutators such as `addOrIncrement`, `decrementItem`, and `removeItem`. | `LL/src/Presentation/ll/src/app/core/services/api/inventory/inventory-state.service.ts` | This is the only confirmed feature store applying SignalR state. It also manually mutates inventory after commands, which can drift from server truth. |
...
| Colosseum state | `ColosseumService` uses `BehaviorSubject<ArenaTicketStatus | null>` for ticket status and returns combat via `CombatService`; no dedicated signal state service. | `LL/src/Presentation/ll/src/app/core/services/api/colosseum/colosseum.service.ts` | Ticket status is loaded by separate GET. Start battle response does not update it. |
| Colosseum component state | `ColosseumComponent` stores opponents, rankings, previous matches, and result flags directly; `ArenaBattleComponent` decrements `arenaTicketStatus.currentTickets--` locally before emitting challenge. | `LL/src/Presentation/ll/src/app/features/game/city/colosseum/colosseum.component.ts`, `LL/src/Presentation/ll/src/app/features/game/city/colosseum/arena-battle/arena-battle.component.ts` | This explicitly violates the target rule. Ticket count should come from `StartArenaBattle` response. |
...
| Marketplace state | Signal-based `MarketplaceStateService` loads listings and manually removes/decrements listings after boolean commands. Components also decrement inventory after creating listings and decrement listings after buyout. | `LL/src/Presentation/ll/src/app/core/services/api/market-place/market-place-state.service.ts`, `LL/src/Presentation/ll/src/app/features/game/city/market-place/market-place-buy/market-place-buy.component.ts`, `LL/src/Presentation/ll/src/app/features/game/city/market-place/market-place-sell/market-place-sell.component.ts`, `LL/src/Presentation/ll/src/app/features/game/city/market-place/market-place-commodity/market-place-commodity.component.ts` | Buyer state should come from REST; seller notification/update is a good SignalR use case. |
| Guild state | Signal-based `GuildStateService` refreshes snapshots after several actions but also manually mutates donations, upgrade level/cost resources, invites, and guild state. A realtime effect is commented out. | `LL/src/Presentation/ll/src/app/core/services/api/guild/guild-state.service.ts` | Guild group updates are planned conceptually but not implemented. |
| Equipment state | Signal-based `EquipmentStateService` mutates slots and inventory immediately around `equipEquipment` and `unequipEquipment` calls. | `LL/src/Presentation/ll/src/app/core/services/api/equipment/equipment-state.service.ts` | Equip/unequip should apply authoritative command result DTOs including equipment slots, inventory changes, and stats. |
...
| Colosseum battle started | Player challenges opponent. | `ColosseumController.StartArenaBattle` -> `StartArenaBattleCommand` -> `ColosseumService.StartArenaBattle()` spends ticket and returns `CombatResultDto`. | `ArenaBattleComponent.onChallenge()` decrements ticket locally, then `ColosseumService.startArenaBattle()` starts combat playback. | REST combat result plus local ticket mutation. | Needs command result DTO including `ArenaTicketStatusDto`; no SignalR echo needed. |
...
| Market listing bought | Player buys listing. | `MarketPlaceController.BuyoutListing` -> `BuyoutMarketPlaceListingCommand` -> `MarketPlaceService.BuyoutMarketPlaceListingAsync()` mutates buyer/seller/listing/inventory. | `MarketplaceStateService.buyoutListing()` returns `Observable<boolean>`; components decrement listing locally. | REST boolean plus local mutation. | Caller should receive buyer inventory/currency/listing update; seller should receive SignalR notification/update if online. |
| Guild donation | Player donates resources. | `GuildController.Donate` -> `DonateToGuildCommand` -> `IGuildService.DonateToGuildAsync`. | `GuildStateService.donate()` manually increments guild resource totals. | REST boolean plus local mutation. | Should return updated guild resources and caller inventory/resource changes. |
...
- Guild and world subscription concepts exist, but guild authorization and join logic are commented out.
...
Current essence loadout save returns `EssenceLoadoutDto`, but activate/ascend/evolve/favorite often return only `ResponseMessageDto`. Recommended response shape:
...
| Dungeon rewards claimed | Player command | REST command response | REST snapshot | Proposed rewards/inventory/character result | None needed | Currently `Response<bool>`. |
...
| Ticket consumed | Player starts battle | REST command response | None | Updated `ArenaTicketStatusDto` | Same-player other tab SignalR optional | Frontend currently decrements locally. |
...
| Essence ascended | Player command | REST command response | REST snapshot | Updated essence/archive/stats | Same-player other tab optional | Currently message-only plus refresh. |
...
| Inventory items removed | Player command | REST command response | REST snapshot | Inventory change/result DTO | Same-player other tab optional | Frontend often decrements locally today. |
...
| Market listing cancelled | Seller command | REST command response | SignalR market group optional | Returned inventory/listing removal result | Market viewers optional | Currently bool. |
...
- Guild subscription authorization is commented out in `GameHub.SubscribeToAudience`.
...
- Identify key player commands returning `Response<bool>` or message-only DTOs.
- Fix REST command responses before adding broad SignalR events.
- Start with Colosseum because it has an obvious ticket-count issue and visible UI.
...
- Add explicit `GameHub` authorization.
- Evolve envelope metadata.
- Add connection status/reconnect callbacks to `GameEventService`.
- Do not broadcast every state change.

### Phase 4: First Realtime Vertical Slice

- Choose a genuine out-of-band update.
- Best candidate after REST cleanup: market seller notification/update when another player buys your listing.
- Alternative: `LootReceivedMsg` hardening because it already exists.
...
- Ensure command responses update local caller state.
- Use SignalR only for out-of-band changes or same-player cross-session updates.
- Add compact inventory/currency/essence update contracts.

### Phase 7: Guild / Market / Notifications / World Systems

- Add authorized guild group updates.
- Add seller market notifications.
- Add a notification center if desired.
- Add world boss/raid pushed updates when those systems exist.

### Phase 8: Hardening

- Add update ids/sequences where needed.
...
`dotnet build LL\LegendsLegacy.sln --no-restore` was run after creating this document. The build did not pass because of existing test compile errors in `LL/tests/EssenceSystem.Tests/EssenceSystemServiceTests.cs`: `FakeDefinitionRepository` and `SingleDefinitionRepository` do not implement `IEssenceDefinitionRepository.GetAllAbilities()`. The command also reported existing warnings, including a high-severity `MessagePack` package advisory warning. No production code was changed by this document.

No production code was modified for this plan.
