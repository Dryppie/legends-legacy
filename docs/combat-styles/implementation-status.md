...
Updated 9 September 2026. Gameplay authority: [Combat Styles game design](game-design.md). Delivery contract: [implementation plan](implementation-plan.md).
...
The page uses the game's shared header, sidebar icon, navigation tabs, and colors, with core mechanic beside the compact Milestones timeline, refinement cards, and upgrade choices. Mastery level, its current bonus, and slots appear inside the header. Conduit's Focus selector sits inside its core mechanic panel. Save/Discard, preview feedback, and the help guide remain available; Page tour and Refresh buttons are removed. Earned XP refreshes through the existing state synchronization without losing the current draft.
...
Focus uses the same shared field dropdown as the Essence and Inventory pages, with Essence-name labels and disabled ineligible options. Cast Order has been removed from the client/API/domain display models and projection. The Cost panel, separate Tradeoff field/catalog copy, and negative-only preview/help/tour callouts have been removed. Descriptions retain factual numerical effects; tuning, actual resource conversion, progression, and XP requirements are unchanged.
...
The separate Core Rank system is replaced by direct mastery-level scaling from 0 through 10. Bastion adds **1% of the base converted Barrier portion per level**; Conduit adds **1 percentage point to charged Focus effects per level**. Level 0 uses the full base mechanic, level 1 gains the first bonus, and level 10 retains the former +10%/+10-point maximum. Zero-Charge Conduit effects and milestone unlock levels remain unchanged.

Current content uses `combat-styles.v4`, retaining the level-based tuning introduced in v3. New snapshots capture level-based tuning; historical committed snapshots use their captured rank-scaled tuning through the compatibility fallback. No earned progression is reset and no schema migration is needed. Current UI and API descriptions use mastery levels and their bonuses rather than a separate rank count. Healing selection, targeting priorities and cast conditions are unchanged.
...
| Verification | Global selection, availability, Focus, progression, migration, boundary, reward and service-provider tests; updated harness request contract and availability assumptions. |
...
- The editor validates Conduit's global Focus against the existing default equipped Essence loadout. Every battle validates that same Focus against its actual equipped Essences. A missing or ineligible Focus produces an actionable configuration error rather than silently changing the style or choosing another Essence.
...
- Snapshots carry resolved tuning, mastery level, choices, Focus identity and content version. Live preparation and snapshot creation share the same style resolution, including both PvP sides.
...
### Focus dropdown and presentation wording
...
- Focused frontend suite: **20 passed**, covering shared-dropdown keyboard selection, disabled/ineligible options, a missing saved Focus, pending-save guards, retained layout, and removed wording.
...
- Authenticated browser check confirmed the textured shared dropdown, name-only options, Focus selection updating the draft/preview, and removal of the Cost panel. Discard restored the initial empty selection; no character changes were saved.
