...
The table retains the original concept seeds. [Reaper](styles/reaper.md), [Shepherd](styles/shepherd.md) and [Gambler](styles/gambler.md) now have developed design proposals with provisional mechanics, per-level bonuses, upgrades and mastery. Reaper is now implemented; Shepherd and Gambler remain proposals. Each style starts at level 0 and follows the shared mastery-level and unlock schedule.
