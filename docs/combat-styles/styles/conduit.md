...
The game checks your Essence's current ability, including its evolution. If the first Essence is unsuitable or the loadout is empty, the battle proceeds with Conduit inactive: none of its effects apply. Conduit remains selected globally and activates again in new battles using an eligible first Essence. This applies to every combat activity, including newly captured defense and tournament loadouts. The game does not skip an unsuitable Essence to channel a later slot. Already committed battles retain their captured Combat Style.
