...
The [attainable-entry review](Blood-Grove-Entry-Review.md) has also completed Fury/Armor Chest variations: 43,200 valid battles across all nine armor outcomes and both weapon conditions, with zero wins in every cell on both seed sets. Its 24 preselected replays show abilities executing while both enemies remain alive at defeat. That increment reached 76 passing harness tests. The scope decision on 8 September 2026 defers Lumo Token replacements and broader composition exploration: the immediate requirement is one viable starter progression path. This cohort remains diagnostic; all builds need not pass its draft goals. The user has since selected Goblin Warrior, Sword and Heavy Chest for Blood Grove and approved a 70% clear-rate aim with a 65–75% working band. The [separate starter reference](Blood-Grove-Starter-Reference.md) records its exact recipe, 400 initial defeats, two matched replays and subsequent failed policy evaluations. With policy coverage added, 79 harness tests now pass. Next, tune that selected recipe's encounters toward the band and confirm a compatible reference with fresh reserved seeds. Reward-box outcomes remain explicit; this wider cohort's goals are still draft.
