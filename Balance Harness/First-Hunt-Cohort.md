...
Next, establish plausible Essence training and Forge states at each checkpoint, then run controlled variations around the Blood Grove failures and the level-10 composition differences. Keep reward-box outcomes and omitted styles explicit. Confirm decisions with a predeclared sample budget/reserved seeds, and approve numerical goals only against the intended player experience.
