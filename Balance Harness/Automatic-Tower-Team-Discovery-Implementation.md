...
**Compact archive increment — 12 September 2026:** the [implementation and comparison review](Tower-Compact-Archive-Review.md) records opt-in `tower-compact-v1`, exact report/ranking/replay parity, 88.69% smaller ordinary archives and 10.31–15.17% lower overall benchmark time. Compact evidence is supported by the existing evaluator, with one verification per shared bundle within an evaluation. Preparation reuse, checkpoint removal, streaming/resumable verification, campaign integration and new statistical stopping rules remain pending under [the remaining exit checks](#next-increment-bulk-performance--planned). The portfolio balance Pass and search-quality Fail are unchanged.
...
Follow the [active plan](Automatic-Tower-Team-Discovery-Plan.md#immediate-next-increment-bulk-performance). The [bounded profiling command](Tower-Performance-Benchmark-Review.md) and [opt-in compact format](Tower-Compact-Archive-Review.md) are implemented and validated. The heading anchor is retained for existing links; the remaining deliverables below are still planned. No persistent prepared-combat worker, campaign migration, resumable verification or staged statistical stopping policy is introduced by the compact increment.
...
| Reusable preparation and persistent workers | Share only immutable content, compiled definitions and seed-independent preparation. Preserve complete ordered Essence builds and actor identities; keep health/status/cooldown/RNG state isolated. Remove discarded playback checkpoint collection from the bulk path without changing Tower result mapping. |
| Bounded, resumable verification | Verify chunk identity, recipe, content/settings/executable, schedule, outcome completeness and required aggregates. Persist receipts bound to immutable digests and validator version; never trust writable paths or timestamps alone. Changed artifacts invalidate receipts, and independent full verification remains possible. |
| Coordinated execution | Preflight the full contract before combat; enforce overall time/storage/combat caps and meaningful completion progress. Benchmark worker counts rather than assuming more is faster. Avoid duplicate verifier passes and concurrent maintenance; preserve completed chunks on cancellation or failure. |
...
Keep storage/execution optimization separate from changes to statistical acceptance. A future staged sampling policy must predeclare complete family coverage, stage budgets, uncertainty allocation and stopping rules; current frozen schedules remain fixed. Preserve the 10–50% target and the user's fixed gear/untrained-Essence budget. After performance validation, resume stronger combination searches and challenger checks before broader-floor competitive acceptance.
