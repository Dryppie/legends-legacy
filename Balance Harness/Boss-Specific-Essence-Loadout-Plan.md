...
The later [v4 pilot](Tower-Party-Coverage-Review.md) measured two newly generated Kharad builds at **89/256 (34.77%)** and **99/256 (38.67%)** under the same applied boss setting. These are separate 16-cell pilot results, not a replacement for the 2,918-party confirmation above. Both builds came from one restart, so search reliability remains **Fail (1/3; 2 required)**. Carry both into the [separate replication](Tower-Coverage-Replication-Plan.md) as reference controls before proceeding to the next floor batch.
