...
The user has since selected **Goblin Warrior + Sword + Heavy Chest for Blood Grove** and approved a **70% clear-rate aim with a 65–75% initial working band**. The [selected starter reference](Blood-Grove-Starter-Reference.md) freezes one-handed Shortsword and Heavy Breastplate at the existing level-5 entry defaults. Its separate 400-battle measurement produced no wins and now fails the scoped policy. Next, investigate a focused adjustment toward that band while keeping the recipe fixed. Fixed-pair defeats do not prove whole-area failure. Preserve this broader matrix as diagnostic evidence and confirm the chosen candidate with fresh reserved seeds before accepting a regression reference. Other cohort goals remain draft; see the [delivery plan](Balance-Harness-Plan-With-Benchmarking.md#next-work-to-complete-phase-2).
