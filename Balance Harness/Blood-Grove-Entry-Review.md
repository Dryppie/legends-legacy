...
The next attainable option to test is the **guaranteed Lumo Essence Token** from [Trial of Lumo](../LL/src/API/API.LL/Data/quests/region-01/trial-of-lumo.v4.json). At character level 5 it must replace the existing Essence in the single available slot, not add a second Essence. Compare its legal choices on the same declared armor/Fury budget, then use the resulting evidence to decide whether the entry experience needs a clearer loadout transition, a reward change or an encounter adjustment. First Hunt retention, later character levels and the level-10 composition gaps should remain separate questions. Numerical goals remain draft.
