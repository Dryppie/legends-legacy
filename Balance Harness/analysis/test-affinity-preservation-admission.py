"""Affinity preservation admission boundary tests; no live history, timing probe or combat."""
import copy
...
spec = importlib.util.spec_from_file_location('preservation_admission', Path(__file__).with_name('prepare-affinity-preservation-admission.py'))
...
class DeclarationTests(unittest.TestCase):
    def declaration(self):
        return dict(copy.deepcopy(a.CONTRACT), harnessFiles={n:a.CONTRACT['harnessSha256'] for n in a.base.HARNESS_FILES},
            ...
            tests=dict(path=str(a.ROOT/'TestResults/affinity-preservation-admission-tests-20260924.log'),sha256='log',passed=26,failed=0))
    ...
    def validate(self, d):
        ...
        with patch.object(a,'sha',side_effect=digest), patch.object(Path,'read_text',return_value='Ran 26 tests in 0.123s\n\nOK\n'):
...
class QualificationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.old = dict(execution=execution,executionHash='old',settingsHash='settings',materializations=176,nativePreparations=176,fights=0,newValues=0)
        self.new = copy.deepcopy(self.old); self.new['execution']['assemblyHashes']['BalanceHarness']=a.CONTRACT['harnessSha256']
        self.new.update(executionHash='new',replayedArms=4,preservingReplayedArms=2,compiledSourceDocuments=227,jitResolvedMethods=['BalanceHarness.'+name+'#1' for name in ('TowerAffinityCreation.Create','TowerProposalComparison.CreateValidationPlan','TowerProposalComparison.ValidateValidationTrajectories','TowerProposalComparison.CreatePreservationPlan','TowerProposalComparison.ValidatePreservationTrajectories','TowerBatchRacing.Select','TowerBenchmarkValidation.Gate','TowerBenchmarkValidation.Decide')])
        self.captured = dict(execution=copy.deepcopy(execution),executionHash='old')
        self.rows = [dict(ordinal=i,seed=12,scenarioHash='recipe',inputHash='input',participantsHash='prepared') for i in range(176)]
    ...
    def test_all_recipes_use_only_two_declared_historical_values_without_mutation(self):
        before=copy.deepcopy(self.export); rows=a.probes(self.export,self.preserving,self.context)
        self.assertEqual(self.export,before); self.assertEqual(len(rows),88)
    ...
    def test_missing_duplicate_or_uneven_arm_coverage_rejected(self):
        ...
        for export in (missing,duplicate,uneven):
            with self.assertRaises(ValueError): a.probes(export,self.preserving,self.context)
    ...
    def test_preserving_probe_requires_both_full_waves_and_the_new_export_version(self):
        for change in (lambda d:d.update(version='tower-proposal-export-v3'),
            ...
            with self.assertRaises(ValueError): a.probes(self.export,bad,self.context)
...
class ForecastTests(unittest.TestCase):
    def setUp(self):
        ...
        self.completed=dict(status='Complete',version='tower-benchmark-validation-comparison-v1',nativeSeconds=1200,auditSeconds=245,nativeBytes=1900,auditBytes=25)
        self.creation=dict(status='Complete',version=a.STUDY,nativeSeconds=260,auditSeconds=55,nativeBytes=632,auditBytes=2.6)
        self.preservation=dict(status='Complete',version='tower-benchmark-validation-comparison-v1',nativeSeconds=300,auditSeconds=63,nativeBytes=620,auditBytes=2.6)

    def result(self):
        return a.forecast(self.previous,self.probe,850,self.estimate,self.completed,16768,self.creation,self.preservation)

    def test_completed_storage_floor_scales_audit_before_admission(self):
        result=self.result(); floor=2*1900*(21888/16768)*(632/620)
