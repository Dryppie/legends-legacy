...
spec = importlib.util.spec_from_file_location('affinity_admission', Path(__file__).with_name('prepare-affinity-preservation-recognition.py'))
...
class AdmissionTests(unittest.TestCase):
    ...
    def test_forecast_uses_worst_time_maximum_report_and_margin(self):
        ...
        self.assertAlmostEqual(2*(1000*37120/15744+10), result['nativeSeconds'])
        self.assertAlmostEqual(2*(200*37120/15744+10), result['auditSeconds'])
        self.assertEqual(2*(37120*21000+100000000+64*1048576), result['nativeBytes'])
    ...
    def test_definition_is_exact_new_profile_without_recipe_redraw(self):
        ...
        self.assertEqual(145, len(d['teams']))
    ...
    def test_request_preserves_recoveries_and_has_only_this_admission_charge(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual((11400, 600, 6979321856, 536870912), (q['maximumSeconds'], q['priorSeconds'], q['maximumBytes'], q['priorBytes']))
    ...
    def test_backend_proof_requires_this_profile_and_all_passes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for replacement in (text.replace('outcome="Passed"', 'outcome="Failed"', 1),
                text.replace('BalanceHarnessAffinityPreservationRecognitionTests', 'WrongProfile')):
    ...
    def population_copy(self, root):
        ...
        shutil.copytree(ROOT/p.POPULATION_HELPER, root/p.POPULATION_HELPER)

    def test_sealed_population_helper_is_required_and_cannot_change(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); self.population_copy(root)
            helper=root/p.POPULATION_HELPER/'builder.py'
