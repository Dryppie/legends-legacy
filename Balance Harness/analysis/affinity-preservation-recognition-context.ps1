...
if ($study.Version -ne 'tower-affinity-preservation-recognition-v1' -or $expected.Version -ne $study.Version) { throw 'Wrong recognition fixture profile.' }
...
try {
    ...
    if ($inputs.Count -ne 290 -or $chunks.Count -ne 145 -or $parties.Count -ne 145) { throw 'Incomplete native transport coverage.' }
...
[BalanceHarness.HarnessJson]::WriteNew[object]((Join-Path $Package 'context.json'),@{
    ...
    literalFixture=@{status='StoredEvidenceReconstructed'; version=$study.Version; teams=$result.Rates.Count; contrasts=$result.Contrasts.Count
        ...
        pairedCells=$result.PairedPool.Cells.Count; pairedRoots=$result.PairedPool.Roots.Count
