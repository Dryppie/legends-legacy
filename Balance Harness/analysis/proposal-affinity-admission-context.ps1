...
try {
    ...
    if ($Mode -eq 'bind') {
        ...
        $plan = [BalanceHarness.TowerProposalComparison]::CreatePlan($context, $old.Candidate)
        Put 'plan.json' $plan
    ...
    if ($Mode -eq 'candidate') {
        ...
        foreach ($name in @('TowerProposalStudy','TowerProposalPolicies','TowerProposalComparison','TowerProposalRacingNative',
