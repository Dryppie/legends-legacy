...
try {
    ...
    $tested = [BalanceHarness.HarnessJson]::Read[Collections.Generic.Dictionary[string,string]]((Join-Path $Implementation 'source-hashes.json'))
    $manifest = [BalanceHarness.HarnessJson]::Read[Collections.Generic.Dictionary[string,string]]((Join-Path $Implementation 'files.json'))
    ...
    foreach ($handle in $reader.Documents) {
        ...
        if ($relative.StartsWith('LL/tools/BalanceHarness/')) {
            if (-not $tested.ContainsKey($relative) -or $tested[$relative] -ne $expected) { throw "Untested symbol source: $relative" }
            $source = Join-Path (Join-Path $Implementation 'sources') $relative
        } else {
            $prefix = [IO.Path]::GetRelativePath($RepositoryRoot, $Implementation).Replace('\', '/') + '/'
            if (-not $relative.StartsWith($prefix)) { throw "Generated source outside sealed implementation: $relative" }
            $key = $relative.Substring($prefix.Length)
            if (-not $manifest.ContainsKey($key) -or $manifest[$key] -ne $expected) { throw "Unbound generated source: $relative" }
            $source = Join-Path $Implementation $key
        }
...
$expected = [BalanceHarness.HarnessJson]::Read[BalanceHarness.TowerFixedFamilyResult]((Join-Path $fixture 'stored-result.json'))
...
try {
    $result = $confirmation.GetMethod('Assess',$flags).Invoke($null,[object[]]@($study,$expected.ArchiveHash))
    ...
    if ($inputs.Count -ne 112 -or $chunks.Count -ne 56 -or $parties.Count -ne 8) { throw 'Incomplete native transport coverage.' }
...
[BalanceHarness.HarnessJson]::WriteNew[object]((Join-Path $Package 'context.json'),@{
    status='CapturedConfirmationRuntimeCompatible'; execution=$execution; executionHash=[BalanceHarness.HarnessJson]::Hash[object]($execution)
    ...
    literalFixture=@{status='StoredEvidenceReconstructed'; version=$study.Version; teams=$result.Rates.Count; contrasts=$result.Contrasts.Count
        qualifiers=$result.QualifyingPartyIds.Count; recommendations=$result.RecommendedPartyIds.Count; samples=6500
