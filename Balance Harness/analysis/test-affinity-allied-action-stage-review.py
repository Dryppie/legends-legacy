...
spec = importlib.util.spec_from_file_location('preservation_stage_review', Path(__file__).with_name('affinity-allied-action-stage-review.py'))
...
class ReviewTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        root = review.ROOT/'TestResults/tower-proposal-owned-fixture-allied-action-20260924/result'
        cls.evidence = review.Evidence(root, '317381ecc45142f9342d77d6d2bc9e227f9de77bbf7053a3faeefc4a47f1319b')
    ...
    def test_wrong_version_rejected(self):
        with self.assertRaisesRegex(ValueError, 'arm contract'):
            self.checked(change=lambda r: r.update(version='tower-proposal-racing-v6'))
    ...
    def test_missing_protected_provider_rejected(self):
        with self.assertRaisesRegex(ValueError, 'allied-action protection'):
            self.checked(change=lambda r: r['batches'][0]['proposals'][0]['affinityCreation']['removalSelection'].update(protectedEssences=['e00', 'e01']))
    ...
    def test_typed_provider_protection(self):
        candidate, control = self.checked(), self.checked('control')
        self.assertEqual(['e02'], candidate['records'][0]['alliedProviders'])
        self.assertTrue(all(not r['removedAlliedProviders'] for r in candidate['records']))
        self.assertEqual(['e02'], control['records'][0]['removedAlliedProviders'])

    def test_forged_provider_metadata_rejected(self):
        for field, value in [('essenceId', 'e09'), ('operation', 'other'), ('target', 'other')]:
            with self.assertRaisesRegex(ValueError, 'allied-action protection'):
                self.checked(change=lambda r: r['batches'][0]['proposals'][0]['affinityCreation']['removalSelection']['alliedActionProtections'][0].update({field:value}))

    def test_provider_inventory_drift_rejected(self):
        inventory = copy.deepcopy(self.context['damageAffinityInventory'])
        provider = review.audit.allied_providers(inventory)[0]
        node = next(n for n in inventory['nodes'] if n['key'] == provider['effectNodeKey'])
        node['definition']['operation'] = 'other'
        with self.assertRaisesRegex(ValueError, 'allied-action effect'):
            review.audit.allied_providers(inventory)

    def test_recipe_occurrences_are_not_distinct_recipes(self):
        arm = self.checked()
        once, twice = review.summarize([arm]), review.summarize([arm, arm])
        self.assertEqual(2*once['accepted'], twice['accepted'])
        self.assertEqual(once['distinctRecipes'], twice['distinctRecipes'])

    def test_heldout_counts_reconstructed_and_drift_rejected(self):
        endpoint = copy.deepcopy(self.result['roots'][0])
        actual = review.heldout_contrasts(self.evidence, endpoint, self.freeze['families'][0])
        self.assertEqual(endpoint['controlWins']-endpoint['benchmarkWins'], actual['control']['netWins'])
        endpoint['controlWins'] -= 1
        with self.assertRaisesRegex(ValueError, 'held-out wins'):
            review.heldout_contrasts(self.evidence, endpoint, self.freeze['families'][0])
