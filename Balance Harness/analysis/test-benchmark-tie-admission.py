"""Selector admission boundary tests; no live history, timing probe or combat."""
import copy
...
spec = importlib.util.spec_from_file_location('selector_admission', Path(__file__).with_name('prepare-benchmark-tie-admission.py'))
...
class DeclarationTests(unittest.TestCase):
    def declaration(self):
        return dict(copy.deepcopy(a.CONTRACT), harnessFiles={n:a.CONTRACT['harnessSha256'] for n in a.base.HARNESS_FILES},
            ...
            tests=dict(path=str(a.ROOT/'TestResults/benchmark-tie-admission-tests-20260923.log'),sha256='log',passed=20,failed=0))
    ...
    def validate(self, d):
        ...
        with patch.object(a,'sha',side_effect=digest), patch.object(Path,'read_text',return_value='Ran 20 tests in 0.123s\n\nOK\n'):
...
class QualificationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.new.update(executionHash='new',replayedArms=4,compiledSourceDocuments=223,jitResolvedMethods=['BalanceHarness.'+name+'#1' for name in ('TowerAffinityCreation.Create','TowerProposalComparison.CreateSelectorPlan','TowerProposalComparison.ValidateSelectorTrajectories','TowerBatchRacing.Select')])
    ...
    def test_runtime_binding_cannot_change_selector_or_generator(self):
        ...
        for field,value in [('version',a.owner.CREATION_VERSION),('selectionContrast',None),
                            ('control',dict(original['control'],name='different')),('roots',13)]:
...
class ForecastTests(unittest.TestCase):
    ...
    def result(self):
        return a.forecast(self.previous,self.probe,850,self.estimate,self.completed,19840,self.creation,self.preservation)

    def test_completed_storage_floor_scales_audit_before_admission(self):
        result=self.result(); floor=2*1900*(21888/19840)*(632/620)
    ...
    def test_preservation_study_cannot_replace_creation_cost_basis(self):
