"""Bounded saved-stage review of the closed endpoint/allied-action preserving pilot.
...
RUN = ROOT/'TestResults/balance/tower-affinity-allied-action-pilot-01-20260924'
RUN_PIN = 'c9d191a8f81439d6bfa947e8c62bd74a6e35352ee5d999b8dcd9ecdfdded54ef'
CLOSEOUT_PIN = 'f58552cbd568e66f6e76090f01cd0be8d450e173b552513d445e97a73379dc82'
ADMISSION = ROOT/'TestResults/affinity-allied-action-admission-20260924'
ADMISSION_PIN = 'c8eb52d39b57fc385a5e88330a8ec621908a3345ef3d12c33360cbdcb361e68d'
PUBLICATION = ROOT/'TestResults/affinity-allied-action-pilot-01-publication-verification-20260924'
PUBLICATION_PIN = '5a1553cf3c0591086514c632ced8c0513ac31cce5ceb8d4dae559156ea8aa7dc'
PRIOR = ROOT/'TestResults/affinity-allied-action-pilot-01-execution-review-20260924'
PRIOR_PIN = 'f694c7b8f0d25414ec60db41614734aae63c23c7af4ecde57497bbdf925f9495'
OUT = ROOT/'TestResults/affinity-allied-action-stage-review-20260924'
TEST_LOG = ROOT/'TestResults/affinity-allied-action-stage-review-tests-20260924.log'
STUDY, SELECTOR = audit.ALLIED_VERSION, audit.VALIDATION_POLICY
...
def route_metadata(proposal, parent, policy, affinities, providers):
    audit.creation_step(proposal, parent, policy, providers)
    ...
    provider_ids = sorted({p['essenceId'] for p in providers if p['essenceId'] in old})
    expected = sorted(set(protect) | (set(provider_ids) if policy['version'] == audit.ALLIED_POLICY else set()))
    require(step['removalSelection']['protectedEssences'] == expected, 'Changed derived endpoint/provider protection')
    return dict(protectedIfPreserving=protect, alliedProviders=provider_ids,
                removedAlliedProviders=sorted(set(provider_ids) & set(step['removed'])), removedCompletableEndpoints=sorted(set(protect) & set(step['removed'])),
...
def arm_review(report, plan, context, policy, affinities, heldout, heldout_seeds, selected, role):
    ...
    version = audit.PRESERVATION_RACING if role == 'control' else audit.ALLIED_RACING
    providers = audit.allied_providers(context['damageAffinityInventory'])
    ...
    for i, panel in enumerate(e['panels']):
        if i in (0, 2):
            ...
            for attempt, proposal in enumerate(batch['proposals'], 1):
                ...
                metadata = route_metadata(proposal, parties[benchmark], policy, affinities, providers)
...
def summarize(arms):
    ...
    return dict(accepted=len(records), distinctRecipes=len({r['id'] for r in records}), attempts=sum(a['attempts'] for a in arms),
        ...
        removedAlliedProviderOccurrences=sum(bool(r['removedAlliedProviders']) for r in records),
        removedEssenceOccurrences=dict(Counter(e for r in records for e in r['edit']['removed'])),
...
def heldout_contrasts(scientific, endpoint, family):
    """Recount saved per-value outcomes, without rerunning or rereading raw battles."""
    rows = {}
    for member in family['members']:
        pid = member['party']['id']
        cell = scientific.read(f"study/heldout-{family['root']:02d}-{member['recipeHash']}.json")
        require(cell['root'] == family['root'] and cell['recipeHash'] == member['recipeHash']
                and len(cell['observations']) == 256, 'Changed held-out cell')
        rows[pid] = []
        for seed, observation in zip(family['seeds'], cell['observations']):
            request, outcome = observation['request'], observation['outcome']
            require(request['seed'] == outcome['seed'] == seed
                    and request['scenario'] == member['scenario']
                    and request['role'] == f"heldout-root-{family['root']:02d}"
                    and outcome['outcome'] in ('Victory', 'Defeat', 'Draw'), 'Changed held-out observation')
            rows[pid].append(outcome)
        require(all(sum(o['outcome'] == 'Victory' for o in rows[pid]) == endpoint[role+'Wins']
                    for role in member['roles']), 'Changed held-out wins')
    return {role: contrast(rows, endpoint[role+'Party'], endpoint['benchmarkParty'])
...
def analyze(scientific, admission):
    ...
    require(result == scientific.read('independent-audit.json')['result'] and result['version'] == design['version'] == STUDY
            and result['status'] == 'Verified' and result['decision'] == 'Inconclusive'
    ...
    for name in ('TowerAffinityCreation.cs', 'TowerAdaptiveRacingGenerator.cs', 'TowerProposalPolicies.cs', 'TowerBatchRacing.cs',
                 'TowerBenchmarkValidation.cs', 'TowerAlliedActionComparison.cs', 'TowerAlliedActionProtection.cs'):
    ...
    for endpoint, family in zip(result['roots'], freeze['families']):
        ...
        contrasts = heldout_contrasts(scientific, endpoint, family)
        require((contrasts['candidate']['netWins']-contrasts['control']['netWins'])/256 == endpoint['methodDifference']
                and contrasts['candidate']['netWins']/256 == endpoint['benchmarkDifference'], 'Changed held-out difference')
        pairs.append(dict(root=number, changedPositions=changes, overlap=overlap, selectedHeldoutContrasts=contrasts,
    ...
    require(len(used_seeds) == 4380 and arm_summaries['control']['gatePasses'] == 2 and arm_summaries['candidate']['gatePasses'] == 0
            and result['differingRoots'] == 2 and result['novelRoots'] == 0
            and arm_summaries['candidate']['selectedCategories'] == {'benchmark': 12}
            and result['benchmark'] == dict(mean=0, standardDeviation=0, lower=0, upper=0)
            and audit.close(result['method'], audit.interval([p['heldout']['methodDifference'] for p in pairs])), 'Changed all-root endpoint')
    ...
    return dict(originalDecision=result['decision'], sourceFights=result['fights'], summary=summary, armSummaries=arm_summaries, pairs=pairs,
        essenceNames={e['id']: e.get('name', e['id']) for e in context['damageAffinityInventory']['essences']},
        providerEvidence=audit.allied_providers(context['damageAffinityInventory']),
...
def run():
    ...
    code = {p: p.read_bytes() for p in (Path(__file__), Path(audit.__file__), Path(__file__).with_name('test-affinity-allied-action-stage-review.py'))}
    ...
    def save(name, value):
        ...
        with (OUT/name).open('xb') as stream:
            stream.write(raw)
    save('declaration.json', dict(kind='ReadOnlyAffinityAlliedActionStageReview', chargedSeconds=SECONDS, chargedBytes=BYTES,
    ...
    try:
        ...
        for name, pin in prior_files.items():
            require((PRIOR/name).resolve().is_relative_to(PRIOR) and audit.sha(PRIOR/name) == pin, 'Changed prior handoff member')
        ...
        require(audit.sha(RUN/'closeout.json') == CLOSEOUT_PIN and all(p.read_bytes() == raw for p, raw in code.items())
                and TEST_LOG.read_bytes() == log and all(audit.sha(PRIOR/name) == pin for name, pin in prior_files.items()), 'Review inputs changed')
        review.update(version='tower-affinity-allied-action-stage-review-v1', newFights=0, newValues=0,
            ...
            recordedBeforeReviewSeconds=prior['cumulativeRecordedSeconds'],
            recordedBeforeReviewBytes=prior['cumulativeRecordedBytes'],
