"""Creation admission boundary tests; no live history, timing probe or combat."""
import copy
...
spec = importlib.util.spec_from_file_location('creation_admission', Path(__file__).with_name('prepare-affinity-creation-admission.py'))
...
class DeclarationTests(unittest.TestCase):
    def declaration(self):
        return dict(copy.deepcopy(a.CONTRACT), harnessFiles={n:a.CONTRACT['harnessSha256'] for n in a.base.HARNESS_FILES},
            ...
            tests=dict(path=str(a.ROOT/'TestResults/affinity-creation-admission-tests-20260923.log'),sha256='log',passed=20,failed=0))
...
class QualificationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.new.update(executionHash='new',replayedArms=4,compiledSourceDocuments=223,jitResolvedMethods=['BalanceHarness.TowerAffinityCreation.Create#1'])
...
class ForecastTests(unittest.TestCase):
    ...
    def result(self):
        return a.forecast(self.previous,self.probe,850,self.estimate,self.completed,17536,self.creation,self.preservation)

    def test_completed_storage_floor_scales_audit_before_admission(self):
        result=self.result(); floor=2*1900*(21888/17536)*(632/620)
...
class EvidenceTests(unittest.TestCase):
    def setUp(self):
        self.temp=tempfile.TemporaryDirectory(prefix='creation-admission-evidence-'); self.addCleanup(self.temp.cleanup)
