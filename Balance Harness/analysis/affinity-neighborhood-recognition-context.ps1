...
if ($study.Version -ne 'tower-affinity-neighborhood-recognition-v1' -or $expected.Version -ne $study.Version) { throw 'Wrong recognition fixture profile.' }
...
try {
    ...
    foreach ($chunk in $chunks) {
        foreach ($seed in @($chunk.Scenario.Seeds[0],$chunk.Scenario.Seeds[$chunk.Scenario.Seeds.Count-1])) {
            ...
            $prepared = $native.PrepareAsync($input,[Threading.CancellationToken]::None).GetAwaiter().GetResult()
            if ($null -eq $prepared) { throw 'Native preparation returned no encounter.' }
    ...
    if ($inputs.Count -ne 264 -or $chunks.Count -ne 132 -or $parties.Count -ne 44) { throw 'Incomplete native transport coverage.' }
...
[BalanceHarness.HarnessJson]::WriteNew[object]((Join-Path $Package 'context.json'),@{
    ...
    literalFixture=@{status='StoredEvidenceReconstructed'; version=$study.Version; teams=$result.Neighborhood.Wins.Count; endpoints=$result.Neighborhood.Endpoints.Count; samples=2048
    ...
    nativePreparations=$inputs.Count; fights=0; newValues=0
})
