"""Bounded read-only verification of the allied-action comparison and its complete live history.
...
VERSION = 'tower-affinity-allied-action-comparison-v1'
...
def verification_accounting(prior, receipt):
    ...
    result = {}
    require(prior.get('status') == 'VerifiedAlliedActionAdmissionNoReservation', 'Missing admitted accounting ledger')
...
def validate_execution(d, prior, admission, study, output):
    require(d['version'] == 'tower-affinity-allied-action-execution-v1' and d['studyVersion'] == VERSION
    ...
    require(prior['status'] == 'VerifiedAlliedActionAdmissionNoReservation'
...
def main(args):
    ...
    prior_path = prior_root/'verification.json'
    require(sha(prior_path) == prior_manifest[prior_path.relative_to(prior_root).as_posix()], 'Changed prior accounting receipt')
    ...
    save('declaration.json',dict(kind='ReadOnlyPublishedAlliedActionVerification', version=VERSION,
