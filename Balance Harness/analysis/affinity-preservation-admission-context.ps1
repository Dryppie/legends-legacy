...
try {
    ...
    if ($Mode -eq 'bind') {
        ...
        $plan = [BalanceHarness.TowerProposalComparison]::CreatePreservationPlan($context, $old.Candidate)
        Put 'plan.json' $plan
    ...
    if ($Mode -eq 'candidate') {
        ...
        foreach ($name in @('TowerAffinityCreation','TowerBenchmarkValidation','TowerAdaptiveRacingGenerator','TowerBatchRacing','TowerProposalStudy','TowerProposalPolicies','TowerProposalComparison','TowerProposalRacingNative',
