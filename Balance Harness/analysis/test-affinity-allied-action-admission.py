"""Allied-action admission boundary tests; no live history, timing probe or combat."""
import copy
...
spec = importlib.util.spec_from_file_location('allied_action_admission', Path(__file__).with_name('prepare-affinity-allied-action-admission.py'))
...
class DeclarationTests(unittest.TestCase):
    def declaration(self):
        return dict(copy.deepcopy(a.CONTRACT), harnessFiles={n:a.CONTRACT['harnessSha256'] for n in a.base.HARNESS_FILES},
            ...
            tests=dict(path=str(a.ROOT/'TestResults/affinity-allied-action-admission-tests-20260924.log'),sha256='log',passed=28,failed=0))

    def validate(self, d):
        def digest(path):
            if path == a.PLAN: return a.CONTRACT['originalPlanFileSha256']
            if path == a.DESIGN: return a.CONTRACT['designFileSha256']
            if path.parent == a.BUILD: return a.CONTRACT['harnessSha256']
            if path.suffix == '.log': return 'log'
            return 'implementation'
        original_read=Path.read_text
        def text(path,*args,**kwargs):
            return 'Ran 28 tests in 0.123s\n\nOK\n' if path.suffix == '.log' else original_read(path,*args,**kwargs)
        with patch.object(a,'sha',side_effect=digest), patch.object(Path,'read_text',text):
    ...
    def test_scientific_identity_limits_paths_and_retry_policy_are_fixed(self):
        for key,value in [('studyVersion',a.owner.VERSION),('chargedSeconds',901),('scientificMaximumBytes',7000000000),
                ...
                ('admissionOnly',False),('output','other'),('scopeId','other'),('precedingRecordedSeconds',0)]:
...
class QualificationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.old = dict(execution=execution,executionHash='old',settingsHash='settings',materializations=1136,nativePreparations=1136,fights=0,newValues=0)
        self.new = copy.deepcopy(self.old); self.new['execution']['assemblyHashes']['BalanceHarness']=a.CONTRACT['harnessSha256']
        self.new.update(executionHash='new',replayedArms=4,preservingReplayedArms=2,alliedReplayedRoots=12,alliedReplayedArms=24,compiledSourceDocuments=227,jitResolvedMethods=['BalanceHarness.'+name+'#1' for name in ('TowerAffinityCreation.Create','TowerProposalComparison.CreateValidationPlan','TowerProposalComparison.ValidateValidationTrajectories','TowerProposalComparison.CreatePreservationPlan','TowerProposalComparison.ValidatePreservationTrajectories','TowerProposalComparison.CreateAlliedActionPlan','TowerAlliedActionProtection.Create','TowerBatchRacing.Select','TowerBenchmarkValidation.Gate','TowerBenchmarkValidation.Decide')])
        self.captured = dict(execution=copy.deepcopy(execution),executionHash='old')
        self.rows = [dict(ordinal=i,seed=12,scenarioHash='recipe',inputHash='input',participantsHash='prepared') for i in range(1136)]
        ...
        self.preserving = dict(version='tower-proposal-export-v4',arms=[dict(name=name,status='Complete',
            teams=[dict(scenario=dict(id=str(i),seeds=[],party=['fixed'])) for i in range(20)],
            batch=dict(candidates=list(range(9))),secondWave=dict(batch=dict(candidates=list(range(8)))))
            for name in ('benchmark-affinity-creation-v3','benchmark-preserving-affinity-creation-v4')])

        self.allied=[]
        for _ in range(12):
            preview=copy.deepcopy(self.preserving); preview['version']='tower-proposal-export-v5'
            for arm,name in zip(preview['arms'],('benchmark-preserving-affinity-creation-v4','benchmark-allied-action-affinity-creation-v5')):
                arm['name']=name
            self.allied.append(preview)
    ...
    def test_all_recipes_use_only_two_declared_historical_values_without_mutation(self):
        before=copy.deepcopy(self.export); rows=a.probes(self.export,self.preserving,self.allied,self.context)
        self.assertEqual(self.export,before); self.assertEqual(len(rows),568)
    ...
    def test_missing_duplicate_or_uneven_arm_coverage_rejected(self):
        ...
        for export in (missing,duplicate,uneven):
            with self.assertRaises(ValueError): a.probes(export,self.preserving,self.allied,self.context)

    def test_complete_native_qualification_accepts(self): self.qualify()

    def test_preserving_probe_requires_both_full_waves_and_the_new_export_version(self):
        for change in (lambda d:d.update(version='tower-proposal-export-v3'),
                       lambda d:d['arms'].pop(), lambda d:d['arms'][1].update(name=d['arms'][0]['name']),
                       lambda d:d['arms'][1]['teams'].pop(), lambda d:d['arms'][0].update(status='Incomplete'),
                       lambda d:d['arms'][1]['secondWave']['batch']['candidates'].pop()):
            bad=copy.deepcopy(self.preserving); change(bad)
            with self.assertRaises(ValueError): a.probes(self.export,bad,self.allied,self.context)

    def test_all_twelve_allied_roots_and_both_complete_waves_are_required(self):
        for edit in (lambda d:d.pop(), lambda d:d[-1].update(version='tower-proposal-export-v4'),
                     lambda d:d[-1]['arms'].pop(), lambda d:d[-1]['arms'][1].update(status='Incomplete'),
                     lambda d:d[-1]['arms'][1]['teams'].pop(),
                     lambda d:d[-1]['arms'][1]['secondWave']['batch']['candidates'].pop()):
            bad=copy.deepcopy(self.allied);edit(bad)
            with self.assertRaises(ValueError):a.probes(self.export,self.preserving,bad,self.context)

    def test_allied_replay_cannot_be_replaced_by_legacy_replay(self):
        for field,value in [('alliedReplayedRoots',11),('alliedReplayedArms',23)]:
            with self.assertRaises(ValueError):self.qualify(new=dict(self.new,**{field:value}))

    def test_preserving_replay_is_required_in_addition_to_legacy_replay(self):
        with self.assertRaises(ValueError): self.qualify(new=dict(self.new,preservingReplayedArms=1))
...
class ForecastTests(unittest.TestCase):
    def setUp(self):
        ...
        self.completed=dict(status='Complete',version='tower-affinity-preservation-comparison-v1',nativeSeconds=1200,auditSeconds=245,nativeBytes=1800,auditBytes=25)
        self.creation=dict(status='Complete',version=a.STUDY,nativeSeconds=260,auditSeconds=55,nativeBytes=632,auditBytes=2.6)
        self.preservation=dict(status='Complete',version='tower-affinity-preservation-comparison-v1',nativeSeconds=300,auditSeconds=63,nativeBytes=620,auditBytes=2.6)

    def result(self):
        return a.forecast(self.previous,self.probe,850,self.estimate,self.completed,15744,self.creation,self.preservation)

    def test_completed_storage_floor_scales_audit_before_admission(self):
        result=self.result(); floor=2*1800*(21888/15744)*(632/620)
    ...
    def test_other_study_cannot_replace_validation_cost_basis(self):
        with self.assertRaises(ValueError):
            a.forecast(self.previous,self.probe,850,self.estimate,self.completed,17536,self.creation,self.preservation)

    def test_relabelled_completed_or_fixture_costs_are_rejected(self):
        for name in ('completed','creation','preservation'):
            saved=getattr(self,name); setattr(self,name,dict(saved,version='different'))
            with self.subTest(source=name), self.assertRaises(ValueError): self.result()
            setattr(self,name,saved)


class EvidenceTests(unittest.TestCase):
    def setUp(self):
        self.temp=tempfile.TemporaryDirectory(prefix='preservation-admission-evidence-'); self.addCleanup(self.temp.cleanup)
    ...
    def test_package_indexed_implementation_proof_is_authenticated(self):
        verification=self.root/'verification'; verification.mkdir()
        a.save(verification/'completion.json',dict(status='proof'))
        a.save(verification/'files.json',{'completion.json':a.sha(verification/'completion.json')})
