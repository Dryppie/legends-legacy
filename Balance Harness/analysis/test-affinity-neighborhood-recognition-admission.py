...
spec = importlib.util.spec_from_file_location('affinity_admission', Path(__file__).with_name('prepare-affinity-neighborhood-recognition.py'))
...
class AdmissionTests(unittest.TestCase):
    def inputs(self):
        ...
        return native, closed, sizes, dict(nativeBytes=60000000, auditBytes=1000000), 10, {k:dict(seconds=200,bytes=500000) for k in native}, 12000000, 60

    def test_forecast_uses_worst_time_maximum_report_and_margin(self):
        result = p.forecast(*self.inputs())
        self.assertAlmostEqual(2*(1000*90112/16256+10), result['nativeSeconds'])
        self.assertAlmostEqual(2*(200*90112/16256+10), result['auditSeconds'])
        self.assertEqual(2*(90112*21000+100000000+64*1048576), result['nativeBytes'])
    ...
    def test_forecast_native_storage_cap(self):
        args = self.inputs(); args[3]['nativeBytes'] = 9*1073741824
        with self.assertRaises(ValueError): p.forecast(*args)

    def test_forecast_audit_storage_cap(self):
        args = self.inputs(); args[3]['auditBytes'] = 600*1048576
        with self.assertRaises(ValueError): p.forecast(*args)
    ...
    def test_postpublication_caps_cannot_borrow_and_reject_invalid_measurements(self):
        for key, value in [('seconds', 500), ('bytes', 30000000), ('seconds', float('nan')), ('bytes', -1)]:
            args = self.inputs(); args[5]['pilot'][key] = value
            with self.assertRaises(ValueError): p.forecast(*args)

    def test_postpublication_accounts_for_current_request_and_fixture_floor(self):
        args = list(self.inputs()); args[7] = 1200
        result = p.forecast(*args)
        self.assertEqual(2420, result['postpublicationSeconds'])
        self.assertAlmostEqual(2*(500000*90112/16256+12000000+16*1048576), result['postpublicationBytes'])

    def test_forecast_uses_larger_nonreport_overhead_from_either_real_source(self):
        args = self.inputs(); args[0]['pilot']['observedBytes'] += 100000000
        args[1]['pilot']['retainedBytes'] += 100000000
        result = p.forecast(*args)
        self.assertEqual(200000000, result['nonReportNativeBytes'])

    def test_request_explicitly_retains_both_nullable_history_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            for name in ('admission-charge.json','definition.json','auditor.py'): (root/name).write_text('{}')
            prior = dict(pendingHistoryRecoveries=None,recoveryReceiptHashes=None)
            q = p.request(root,{},prior)
            p.launcher.validate_request(q)
            for key in prior:
                self.assertIn(key,q); self.assertIsNone(q[key])
                bad = dict(q); bad.pop(key)
                with self.assertRaises(ValueError): p.launcher.validate_request(bad)
    ...
    def test_definition_is_exact_new_profile_without_recipe_redraw(self):
        ...
        self.assertEqual(44, len(d['teams']))
        self.assertEqual([-987, 10], d['excludedCombatSeeds'])
        self.assertEqual([t['scenario'] for t in plan['teams']], [t['scenario'] for t in d['teams']])
        self.assertEqual(plan['sourceContract']['capturedRuntime']['contentHashes'], d['contentHashes'])
    ...
    def test_request_preserves_recoveries_and_has_only_this_admission_charge(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual((19800, 1800, 18790481920, 536870912), (q['maximumSeconds'], q['priorSeconds'], q['maximumBytes'], q['priorBytes']))
    ...
    def test_backend_proof_requires_this_profile_and_all_passes(self):
        path = p.VERIFICATION/'neighborhood-verified-tests.trx'
        p.validate_tests(path)
        with tempfile.TemporaryDirectory() as tmp:
            changed = Path(tmp)/'tests.trx'
            text = path.read_text(encoding='utf-8-sig')
            for replacement in (text.replace('outcome="Passed"', 'outcome="Failed"', 1),
                text.replace('BalanceHarnessAffinityNeighborhoodRecognitionTests', 'WrongProfile')):
    ...
    def test_exact_new_plan_and_captured_source_bindings(self):
        ...
        self.assertEqual(44, plan['population']['physicalRecipes'])
        self.assertEqual(plan['sourceFiles'][p.PILOT.relative_to(ROOT).as_posix()+'/files.json'], p.PINS['pilot'][1])
    ...
    def test_sealed_population_helper_is_required_and_cannot_change(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); self.population_copy(root)
            helper=root/'population/plan/affinity-neighborhood-recognition-plan.py'
            helper.write_text('changed')
            with self.assertRaises(ValueError): p.population(root)

    def test_source_publication_review_and_captured_execution_are_bound(self):
        records=[p.read(p.PLAN),p.read(p.PINS['publication'][0]/'verification.json'),
            p.read(p.PINS['review'][0]/'review.json'),p.read(p.PILOT/'result.json'),p.read(p.CAPTURE/'candidate-qualification.json')]
        p.validate_source_records(*records)
        for index,field,value in [(1,'scientificManifestSha256','0'*64),(1,'scientificFights',15743),
            (2,'originalDecision','Promoted'),(3,'version',p.VERSION),(4,'executionHash','0'*64)]:
            changed=copy.deepcopy(records); changed[index][field]=value
            with self.assertRaises(ValueError): p.validate_source_records(*changed)
    ...
    def test_changed_source_bytes_or_plan_fail(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp); self.population_copy(root)
            entropy = root/'population/source/TestResults/affinity-allied-action-preview-20260924-v2/root-01/batches.json'
    ...
    def test_admission_failure_preserves_charge_and_cannot_retry(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(p, 'PACKAGE', package), patch.object(p, 'OUTPUT', output), patch.object(p, 'validate_declaration'), \
                ...
                self.assertEqual(1800, p.read(package/'failure.json')['chargedSeconds'])
