"""Freeze an affinity-creation recognition diagnostic from the closed v5 pilot.
...
REVIEW = ROOT/'TestResults/benchmark-validation-stage-review-20260924'
REVIEW_PIN = 'bde705235252a4afd04cad3ce5256060e5453422656e209da7ccb8d91ed452f9'
VERSION = 'tower-affinity-creation-recognition-plan-v1'
SOURCE_VERSION = 'tower-benchmark-validation-comparison-v1'
RACING_VERSION = 'tower-proposal-racing-v5'
SELECTION_VERSION = 'tower-racing-benchmark-validation-v1'
...
def source_tools():
    ...
    spec = importlib.util.spec_from_file_location('recognition_source_review', REVIEW/'benchmark-validation-stage-review.py')
...
def root_catalogue(number, report, plan, references):
    ...
    require(plan['version'] == report['version'] == e['version'] == RACING_VERSION
            and plan['selectionPolicyVersion'] == report['selectionPolicyVersion'] == SELECTION_VERSION
            and report['planHash'] == e['planHash'] and len(e['panels']) == len(plan['racing']['panels']) == 6,
            'Changed affinity-creation source contract')
    ...
    return dict(root=number, sourcePlanHash=report['planHash'],
        sourceReport=f'study/pair-{number:02d}.json', sourceReportRole='candidate',
        sourcePlan=f'search/root-{number:02d}/candidate/racing/plan.json',
...
def catalogue():
    ...
    publication = source.Evidence(source.PUBLICATION, source.PUBLICATION_PIN)
    ...
    require(receipt['status'] == 'VerifiedPublishedArchiveAndCompleteLiveHistory'
            and receipt['scientificManifestSha256'] == source.RUN_PIN
            and receipt['scientificCloseoutSha256'] == source.CLOSEOUT_PIN
            and receipt['admissionManifestSha256'] == source.ADMISSION_PIN
            and sha((source.RUN/'closeout.json').read_bytes()) == source.CLOSEOUT_PIN
            and result == evidence.read('independent-audit.json')['result']
            and result['version'] == SOURCE_VERSION and result['status'] == 'Verified'
            and result['decision'] == review['originalDecision'] == 'Inconclusive'
            and review['version'] == 'tower-benchmark-validation-stage-review-v1'
            and result['fights'] == review['sourceFights'] == 16768, 'Incomplete or changed source study')
    context, design = evidence.read('source/context.json'), evidence.read('source/plan.json')
    require(design['version'] == SOURCE_VERSION and design['roots'] == len(result['roots']) == len(review['pairs']) == 12,
            'Changed source cohort')
    source.audit.validate_policies(design)
    template = context['scope']
    references = [s['party']['id'] for s in template['starts']]
    require(set(references) == source.kernel.REFERENCES and references[0] == source.kernel.PRIMARY
            and references[2] == source.kernel.BENCHMARK, 'Changed controls')
    roots = []
    for number, (endpoint, reviewed) in enumerate(zip(result['roots'], review['pairs']), 1):
        pair = evidence.read(f'study/pair-{number:02d}.json')
        plan = evidence.read(f'search/root-{number:02d}/candidate/racing/plan.json')
        report = pair['candidate']
        require(number == endpoint['root'] == reviewed['root'] and pair['status'] == 'Complete'
                and pair['planHash'] == result['planHash'] and reviewed['heldout'] == endpoint
                and plan['policy'] == design['candidate'] and report['policyHash'] == digest(plan['policy'])
                and plan['racing']['mechanics'] == context['mechanics']
                and plan['damageAffinityInventory'] == context['damageAffinityInventory'], 'Changed reviewed source root')
        population = root_catalogue(number, report, plan, references)
        generated = [t['partyId'] for t in population['teams'] if t['stratum'] != 'reference']
        require(set(generated) == {r['id'] for r in reviewed['candidate']['records']}
                and report['evaluation']['nominees'] == reviewed['candidate']['nomineeOrder']
                and report['evaluation']['decisions'][-1]['beamIds'] == [r['id'] for r in reviewed['candidate']['finalBeam']],
                'Catalogue disagrees with authenticated stage review')
        roots.append(population)
    # Every consumed scientific record must also be the exact record inspected
    # by the sealed stage diagnosis. Outcomes authenticate provenance, but never
    # choose catalogue strata or the lower-stratum probability sample.
    ...
    return dict(version=VERSION, status='PopulationFrozenBeforeSampling', runnableRequest=False,
        ...
        sources=dict(scientificManifestSha256=source.RUN_PIN, stageReviewManifestSha256=REVIEW_PIN,
            scientificCloseoutSha256=source.CLOSEOUT_PIN, publicationManifestSha256=source.PUBLICATION_PIN,
            admissionManifestSha256=source.ADMISSION_PIN, consumedScientificFiles=evidence.consumed,
            consumedPublicationFiles=publication.consumed),
        sourceContract=dict(studyVersion=SOURCE_VERSION, racingVersion=RACING_VERSION,
            selectionPolicyVersion=SELECTION_VERSION, generatorPolicy=design['candidate'],
            damageAffinityInventoryHash=digest(context['damageAffinityInventory']),
            scope='Frozen affinity-creation cohort from the candidate arm; both arms share generation and racing. No selector fitting or replay.'),
...
def build_plan(value, selection, catalogue_hash, sampling_pin):
    ...
    return dict(version=VERSION, status='FrozenDesignNativeIntegrationRequired', runnableRequest=False,
        catalogueSha256=catalogue_hash, samplingManifestSha256=sampling_pin, sources=value['sources'], capturedRuntime=value['capturedRuntime'], sourceContract=value['sourceContract'],
        ...
        requiredBeforeExecution=['A separately versioned native/audit profile for this affinity-creation plan and captured runtime; the existing recognition profile pins the earlier adaptive-cohort plan and must continue rejecting this one',
...
def verify_plan(package, pin):
    ...
    return dict(status='VerifiedAffinityCreationRecognitionPlan', newFights=0, newCombatValues=0)
