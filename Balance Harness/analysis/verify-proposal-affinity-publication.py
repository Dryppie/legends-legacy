"""Bounded read-only verification of a sealed proposal study and its live history.
...
def history_summary(ledger, allocation, history_files, values, required_history, study):
    ...
    require(ledger['reservationState'] == 'Complete' and historical == sorted(set(historical))
            and reserved == sorted(set(reserved)) and not set(historical).intersection(reserved)
            and 3948 <= len(reserved) <= 16384 and allocation['reserved'] == reserved
            and len(selected) == len(set(selected)) == 3948 and set(selected) <= set(reserved),
...
def main(args):
    ...
    save('declaration.json',dict(kind='ReadOnlyPublishedProposalVerification', chargedSeconds=SECONDS, chargedBytes=BYTES,
    ...
    try:
        ...
        require(Path(q['outputRoot']) == study and Path(q['registryRoot']) == study.parent
