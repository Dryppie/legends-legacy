...
**Compact archive increment — 12 September 2026:** the [compact archive review](Tower-Compact-Archive-Review.md) records implemented opt-in storage, verification/replay and evaluator support. The fixed 1,200-combat comparison produced identical results/replays, **88.69% smaller ordinary archives** and **10.31–15.17% lower total elapsed time**. Next implement immutable preparation reuse and remove unused bulk playback checkpoints, then bounded/resumable verification and campaign integration. Existing search/calibration drivers still use their current archives. Keep the fixed gear/untrained-Essence budget and 10–50% policy; larger searches and floors 6–11 remain queued until the broader performance prerequisite is complete.
...
| Bulk-performance prerequisite — partly implemented | Bounded profiling and opt-in compact archives are implemented; reusable immutable preparation, bounded/resumable verification and campaign integration remain | Small frozen legacy/optimized comparisons demonstrate matching outcomes, required search metrics and reproducible throughput/storage gains before another large campaign. |
...
### Immediate next increment: bulk performance
...
2. **Compact storage implemented, opt-in:** [versioned shared archives](Tower-Compact-Archive-Review.md) preserve full non-event reports, required search-fitness fields, exact schedule identity and replay. Legacy readers remain available; production campaign/search drivers are not migrated. Next reuse immutable preparation in persistent workers while isolating every battle's mutable state and RNG, and avoid collecting unused playback checkpoints.
3. Make verification resumable by completed chunks with integrity-bound receipts. Coordinate combat, verification and maintenance under one resource budget; defer compression and broad tests from the combat critical path. Strictly preflight contracts before the first fight.
4. Prove parity, cancellation/resume and tamper detection, then measure repeatable end-to-end speed and storage gains on the fixed benchmark. Do not use another 600,000-fight campaign as the initial performance test. The [implementation guide](Automatic-Tower-Team-Discovery-Implementation.md#next-increment-bulk-performance--planned) defines the exit checks.
5. After that gate, improve independent restart reliability and retained-build challenges at current boss settings. Broaden cheap screening before freezing expensive calibration confirmation, since tuning can change the strongest recipe. Design any [staged sampling policy](Tower-Balance-Acceptance-Policy.md#planned-staged-sampling-and-compact-evidence) separately, before its results exist; do not retrofit it to completed evidence.

The completed Kharad portfolio has a scoped balance **Pass**, while competitive search quality remains **Fail**. Faster execution must support stronger combination discovery; more trials on known recipes alone do not establish near-optimality. The later floor batch below remains queued behind this prerequisite.
