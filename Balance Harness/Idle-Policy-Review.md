...
Full local bundles are ignored under `TestResults/balance/phase2-smoke-validation`, `phase2-reference-review` and `phase2-reference-history`. Retain them if this local review needs to be audited; they are not committed or guaranteed to exist in another checkout. Hosted execution had not yet been observed at this increment; the [later acceptance review](Starter-Path-Acceptance.md#hosted-ci-and-local-verification) records the first passing run on published main. No deployment, database migration or gameplay configuration change was part of this work.

## Subsequent decisions

The separately versioned First Hunt cohort covers the three actual choices, guaranteed starter weapons, fixed possible quest-box outcomes and two-enemy later encounters. The follow-up [Blood Grove progression review](Blood-Grove-Progression-Review.md) tested unascended levels 1/10 and gear ranks 0/1/5: every cell lost on both seed sets, and training alone changed no combat summaries. The [attainable-entry review](Blood-Grove-Entry-Review.md) also found no wins after testing Fury and all nine Armor Chest outcomes. The scope decision on 8 September 2026 defers Lumo Token replacements: require a viable starter progression path, with other Essence results remaining diagnostic. The user selected Goblin Warrior, Sword and Heavy Chest for Blood Grove, retaining a 70% aim and later widening the accepted band to **50–90%**. Its [separate reference](Blood-Grove-Starter-Reference.md) and [policy revision](Blood-Grove-Band-Review.md) retain the initial failures and uncertainty.

By 9 September, both selected starter checkpoints had accepted local baselines, a passing [combined regression](Starter-Regression-Review.md) on the retained 8 September build, and a [verified local recovery package](Starter-Baseline-Package.md). Those later results do not change this review's original measurements or draft policies. Off-device storage is deferred by user choice. Broader target approval and hosted verification of unpublished changes remain open; the [main plan](Balance-Harness-Plan-With-Benchmarking.md#phase-2-integration-and-follow-up) records current priorities.
