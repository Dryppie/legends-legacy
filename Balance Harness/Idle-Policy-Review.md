...
The separately versioned First Hunt cohort now covers the three actual choices, guaranteed starter weapons, fixed possible quest-box outcomes and two-enemy later encounters. Use its results to review plausible training/Forge states and the resulting level-5 failures before tuning content or approving goals. Keep the original controls for regression comparisons. Phase 2's advisory workflow is implemented; gameplay-target approval and hosted CI validation remain open.
