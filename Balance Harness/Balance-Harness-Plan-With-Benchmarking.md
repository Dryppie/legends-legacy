...
Status: offline idle execution, the original 12-cell controls, a separate 36-cell [First Hunt cohort](First-Hunt-Cohort.md), explicit baseline acceptance, paired comparisons, versioned goals and advisory CI configuration are implemented in [BalanceHarness](../LL/tools/BalanceHarness/README.md). The selected two-cell [Blood Grove starter reference](Blood-Grove-Starter-Reference.md) now has an approved 70% clear-rate aim with a 65–75% working band. The original cohorts' goals remain draft. A viable starter baseline, other gameplay targets, first hosted CI validation, broader content coverage and tuning remain open.
...
Phases 0 and 1 are implemented for fixed idle encounters, including group combat, victory/defeat replay, legal partial equipment, paired seeds, scorecards and visible cancellation. Phase 2 includes explicit baselines, compatible paired comparisons and distinct evaluation outcomes. Both original cohorts have draft policies, advisory CI configuration and local smoke/reference verification. The selected Blood Grove starter now has a reviewed working clear-rate band and valid failed evaluations of its initial evidence. The First Hunt extension addresses starter choices and enemy counts; the [progression investigation](Blood-Grove-Progression-Review.md) tests unascended training/reinforcement and the [attainable-entry investigation](Blood-Grove-Entry-Review.md) tests Fury/all Armor Chest outcomes with declared budgets and separate confirmation seeds. These are bounded investigations toward Phase 4, not a general build search. Lumo Token replacements and broader Essence exploration are deferred. A viable starter baseline, remaining gameplay targets and observation of the first hosted CI run remain open. Do not derive approved targets automatically from observed results.

### Next work to complete Phase 2

1. Run a focused tuning experiment toward the approved 70% aim (65–75% working band per encounter), keeping the selected Goblin Warrior/Shortsword/Heavy Breastplate recipe fixed. Predeclare the candidate adjustment, bounds, sample budget and unaffected controls before execution. The [initial evidence](Blood-Grove-Starter-Reference.md) is 0/100 wins in each cell on both seed sets, with median defeat times of 30 and 39 seconds. The selected heavy chest remains a fixed reward condition; these pairings do not establish whole-area difficulty.
2. Confirm the chosen candidate on fresh reserved seeds with a predeclared 1,000-trial budget per encounter. A 70/100 estimate is inconclusive against the working band under the existing Wilson-interval rule; 700/1,000 can pass. Do not repeatedly extend samples until a check passes. Verify unaffected controls and preserve the original losing evidence. A recipe change would require a reviewed fixture/policy revision.
3. Accept a compatible regression reference once the evidence supports the scoped policy. The policy for `idle-blood-grove-starter-v1` is now authored and reviewed; the initial discovery and confirmation evaluations each have two valid failed checks. Original controls and the 36-cell cohort remain diagnostic/draft, and their 63 pass, 26 fail and 91 inconclusive findings do not require every build to pass. Other builds, checkpoints and pacing targets remain separate.
4. Observe the first hosted CI run and adjust operational budgets if needed. CI still smoke-tests the two original draft cohorts using three samples per cell, two identical runs each, disposable repeatability references and seven-day retention. The selected starter policy enforces its band only when explicitly evaluated. Adding it as a hosted gameplay gate remains a separate workflow decision.
...
The filter includes all seven harness test classes. Add affected content-specific tests for each new adapter. Use the complete `./build/run-tests.ps1` suite when a change affects shared combat/preparation behavior.
...
Repository references used to ground this plan. The idle tool, reference suites, baseline workflow, goal evaluator, reviewed Blood Grove starter band and advisory smoke/CI configuration exist. Broader player-cohort enforcement, first hosted CI validation and further analysis remain future work.
