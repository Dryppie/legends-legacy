...
Phases 0 and 1 are implemented for fixed idle encounters, including group combat, victory/defeat replay, legal partial equipment, paired seeds, scorecards and visible cancellation. Phase 2 includes explicit baselines, compatible paired comparisons, draft goals and distinct evaluation outcomes with optional reviewed enforcement. Both cohorts have advisory CI configuration and local smoke/reference verification. The First Hunt extension addresses starter choices and enemy counts; the [progression investigation](Blood-Grove-Progression-Review.md) tests unascended training/reinforcement and the [attainable-entry investigation](Blood-Grove-Entry-Review.md) tests Fury/all Armor Chest outcomes with declared budgets and separate confirmation seeds. These are bounded investigations toward Phase 4, not a general build search. Lumo Token replacements, Ascension, the broader progression matrix and gameplay-target approval remain open. The first hosted CI run also needs observation. Do not derive approved targets automatically from observed results.

### Next work to complete Phase 2

1. Test the guaranteed Lumo Token's legal Essence replacements in the single level-5 slot, using the established armor/Fury resource ledger. The [entry findings](Blood-Grove-Entry-Review.md) contain 43,200 valid battles with no wins; selected replays show abilities firing and both enemies surviving the player. Determine whether an attainable Essence transition supplies the missing capability before changing rewards or encounter pressure. Keep First Hunt retention and the level-10 composition investigation separate.
