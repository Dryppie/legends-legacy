...
| 10 | Supporting idle tooling, First Hunt cohort and advisory CI configuration implemented; certification pending | Reproducible idle runs, baselines, draft goals and local verification cover original controls plus actual starter choices and fixed two-enemy encounters. Training/Forge assumptions, the full Beta matrix, reviewed targets, hosted CI validation, player observation and feature freeze remain outstanding. |
...
The separate [First Hunt cohort](Balance%20Harness/First-Hunt-Cohort.md) adds 36 cells/180 draft checks using all three actual starter Essences, mace/wand choices, quest-reward equipment counts and fixed two-enemy later encounters. Two 3,600-battle runs matched; 57 harness tests passed. All tested level-5 untrained builds lost the Blood Grove pairings, so training/Forge states and reward assumptions need investigation before enemy tuning or target approval. CI configuration now covers both cohorts.

The [initial policy review](Balance%20Harness/Idle-Policy-Review.md) identified the original control's starter and encounter-count gaps. The First Hunt extension addresses those selections while retaining fixed possible Armor/Jewelry Chest outcomes and untrained Essences as explicit conditions. All goals remain draft. Hosted CI execution and gameplay review of progression states remain open, as recorded in the [delivery plan](Balance%20Harness/Balance-Harness-Plan-With-Benchmarking.md). This supports Step 10, but does not yet certify the four Beta build directions, the level-1-to-30 journey, dungeon carryover or acquisition pacing. Human journey validation and the wider chapter/build roadmap remain necessary.
