...
| 10 | Supporting idle tooling, First Hunt cohort, progression investigations and advisory CI configuration implemented; certification pending | Reproducible idle runs, baselines and local verification cover original controls plus actual starter choices and fixed two-enemy encounters. Goblin Warrior/Sword/Heavy Chest is the selected Blood Grove reference with an approved 70% aim and 65–75% working band; its initial evidence fails the target, so tuning and a viable baseline remain open. Lumo Token exploration is deferred. The full Beta matrix, broader reviewed targets, hosted CI validation, player observation and feature freeze remain outstanding. |
...
The user-selected [Blood Grove starter reference](Balance%20Harness/Blood-Grove-Starter-Reference.md) pins Goblin Warrior, one-handed Shortsword and Heavy Breastplate at the existing level-5 entry defaults. Its two-cell fixture recorded 0/100 wins against both pairings on discovery and confirmation seeds (400 defeats total); two preselected replays matched. The user subsequently approved an aim near 70%, explicitly choosing an initial 65–75% working band per encounter. The scoped policy records that review and evaluates both saved runs as valid balance failures, with two failed checks and exit code 1 each. All 79 harness tests passed, including policy/recipe coverage and statistical-boundary checks. Next is a focused tuning experiment followed by fresh reserved-seed confirmation before accepting a viable baseline. Production balance and hosted CI configuration remain unchanged.

The [initial policy review](Balance%20Harness/Idle-Policy-Review.md) identified the original control's starter and encounter-count gaps. The First Hunt extension addresses those selections while retaining fixed possible Armor/Jewelry Chest outcomes and untrained Essences as explicit conditions. Goals outside the selected Blood Grove starter policy remain draft. Hosted CI execution and broader progression-state review remain open, as recorded in the [delivery plan](Balance%20Harness/Balance-Harness-Plan-With-Benchmarking.md). This supports Step 10, but does not yet certify the four Beta build directions, the level-1-to-30 journey, dungeon carryover or acquisition pacing. Human journey validation and the wider chapter/build roadmap remain necessary.
