...
## Beta Preparation Progress — Updated 9 September 2026
...
| 10 | Supporting idle tooling, starter regression and portable recovery implemented; wider certification pending | The selected Blood Grove and Crystal Creek checkpoints pass approved 50–90% win bands on the retained 8 September build. Its 52,000 repeated fights match and all 2,119 backend tests passed. Rates remain 89.13% / 76.21% in Blood Grove and 74.40% / 55.45% in Creek. The 9 September local package restores 428,158 files and matches eight replays. Off-device storage is deferred by user choice; hosted verification after publication remains. Lumo Token exploration is deferred. The full Beta matrix, broader coverage, player observation and feature freeze remain outstanding. |

### Balance tooling update — 8 September 2026

The [Balance Harness](LL/tools/BalanceHarness/README.md) runs production idle combat offline across the original 12 fixed cells at levels 1, 5 and 10, with 100 seeds per cell. It saves replayable evidence, produces scorecards, accepts explicit comparison baselines, compares compatible candidates and evaluates six versioned draft goals across 60 checks. Reports distinguish pass, fail, inconclusive and invalid findings. The two original cohort policies remain advisory; the separate selected starter policy below now has a reviewed working target.

The separate [First Hunt cohort](Balance%20Harness/First-Hunt-Cohort.md) adds 36 cells/180 draft checks using all three actual starter Essences, mace/wand choices, quest-reward equipment counts and fixed two-enemy later encounters. Its initial two 3,600-battle runs matched and 57 harness tests passed. All tested level-5 untrained builds lost the Blood Grove pairings, motivating the progression investigation below before enemy tuning or target approval. CI configuration now covers both cohorts.

The follow-up [Blood Grove progression investigation](Balance%20Harness/Blood-Grove-Progression-Review.md) completed 14,400 valid battles across Essence levels 1/10 and both-item ranks 0/1/5 on two fixed seed sets. Every cell still lost; all 7,200 training pairs had identical combat summaries because unascended levels do not directly increase combat strength. Reinforcement costs exceed the guaranteed entry budget. That increment brought the harness to 67 passing tests and preserved both existing cohort smoke workflows. It motivated the Fury/Armor Chest investigation below; goals and combat coefficients remained unchanged.

The [attainable-entry investigation](Balance%20Harness/Blood-Grove-Entry-Review.md) then tested Fury and all nine Armor Chest outcomes across 43,200 battles. Every cell still lost on both seed sets. All 24 preselected replays matched and showed active abilities firing while both enemies survived the player. That increment reached 76 passing harness tests, including production Forge-quote equivalence, and both original cohort smoke workflows passed. The scope decision on 8 September 2026 defers Lumo Token replacements: a viable starter progression path is sufficient, and other Essence results remain diagnostic. Goals, combat coefficients and production rewards remain unchanged.

The user-selected [Blood Grove starter reference](Balance%20Harness/Blood-Grove-Starter-Reference.md) pins Goblin Warrior, one-handed Shortsword and Heavy Breastplate at the existing level-5 entry defaults. Its two-cell fixture recorded 0/100 wins against both pairings on discovery and confirmation seeds (400 defeats total); two preselected replays matched. The user subsequently approved an aim near 70%, explicitly choosing an initial 65–75% working band per encounter. The scoped policy records that review and evaluates both saved runs as valid balance failures, with two failed checks and exit code 1 each. That increment reached 79 passing harness tests and established the target for the tuning investigation below.

The [regional offense experiment](Balance%20Harness/Blood-Grove-Pressure-Review.md) completed 18,200 valid battles across 23 legal bonus values, original/candidate confirmation and both existing control suites. The selected value 0.2 confirmed at 88.4% Raven-pair and 78.6% mixed-pair clears, both above the working band. All 16 Lumo controls stayed identical, but six early Shenic areas changed. Four selected replays matched and 83 harness tests passed. No production balance or viable-baseline promotion occurred.

The separate [fine offense experiment](Balance%20Harness/Blood-Grove-Fine-Pressure-Review.md) completed 24,600 battles across 11 values from 0.30 through 0.20. Selected bonus 0.21 confirmed at 88.5% Raven-pair clears (Fail under the initial policy) and 74.1% mixed-pair clears (Inconclusive under that policy). The discovery jump aligns with integer Raven damage thresholds. All 16 Lumo cells stayed identical, the same six areas changed and four replays matched; that increment reached 85 passing harness tests.

The user then approved a wider **50–90% win-rate band**, retaining the 70% aim. The [v2 policy review](Balance%20Harness/Blood-Grove-Band-Review.md) re-evaluated six saved runs without new battles. Both tuned candidates' observed rates fit the new band; each mixed-pair check passes and each Raven-pair check remains inconclusive because its interval slightly exceeds 90%. That policy-only increment left production balance and hosted CI unchanged and deferred the previous second-parameter tuning recommendation.

The authorized [Blood Grove-only implementation](Balance%20Harness/Blood-Grove-Local-Validation.md) now sets local offense to 2.421 in regional content version 12. Its fixed 21,600-battle validation uses 3,000 fresh trials per encounter: **89.30% Raven clears (88.14–90.36%, Inconclusive)** and **76.73% mixed clears (75.19–78.21%, Pass)**. All 13 other areas retain scaling, all 32 external control cells retain gameplay and four fixed replays match. All 2,066 backend tests pass. No viable baseline was accepted and no sample extension occurred. Next, playtest the recipe and the transition to unchanged Crystal Creek, whose 86.33% offense increase now has an explicit validation ceiling. This backend/content increment adds no migration or hosted CI change and has not been deployed.

The [level-10 starter handoff](Balance%20Harness/Crystal-Creek-Starter-Handoff.md) has a completed 16,000-battle comparison. The user subsequently approved **50–90% wins, with no duration target**, for the primary Amulet/Goblin checkpoint. Its 99.4% / 100% confirmation results fail above the upper bound. [Starter Path Acceptance](Balance%20Harness/Starter-Path-Acceptance.md) records the scoped policy, preserved historical evidence, local playtest and first passing hosted CI run. The current increment passes 108 local harness/style tests and both cohort smokes; no new enemy tuning or passing baseline is inferred.

The subsequent [Crystal Creek creature-pressure experiment](Balance%20Harness/Crystal-Creek-Tuning-Review.md) completed 120,000 battles and remained Inconclusive. Its separately declared [fine follow-up](Balance%20Harness/Crystal-Creek-Fine-Tuning-Review.md) completed **275,200 battles** and selected Barrier 0.44 / Ice Needle 1.7. Confirmation is **74.40% / 55.45%, both Pass**, with both full 95% intervals inside 50–90%. External controls and replays match. The corrected creature-only definitions are local API content after 36,800 same-seed compatibility repetitions with no gameplay changes; player Essence abilities remain unchanged. This scoped Creek baseline preceded the separate Blood Grove acceptance recorded below. No deployment or migration is part of this increment.

The later [Blood Grove acceptance confirmation](Balance%20Harness/Blood-Grove-Acceptance-Confirmation.md) resolves that remaining checkpoint without changing content or policy. A fixed **20,000-battle** run on fresh seed 918091 reaches **89.13% Ravens [88.50–89.73%]** and **76.21% mixed [75.37–77.03%]**, both Pass; two replays match and all **2,112 backend tests pass**. Its local baseline was explicitly accepted, while the original Inconclusive evidence remains closed with no exception or pooled samples. Creek retains its earlier executable identity; that Blood Grove confirmation alone covers only its own checkpoint. No gameplay implementation, migration or deployment was added by this confirmation.

The subsequent [combined starter regression](Balance%20Harness/Starter-Regression-Review.md) verifies both accepted references on the retained 8 September build after shared Combat Styles changes. All **52,000 repeated fights** across eighteen cells match their saved gameplay records and outcomes, all four primary win policies pass, and four detailed replays match. That build passed **2,119 backend tests**. The new `build/check-starter-balance.ps1` preserves the accepted fixtures and schedules for future local checks. No baseline was promoted and no independent samples were added.

The [9 September evidence package](Balance%20Harness/Starter-Baseline-Package.md) now preserves both accepted references, their exact executables and complete supporting evidence in a **1.59 GB ZIP**. All **428,158 files** restore outside the checkout, all four saved suites pass their existing checks, and **eight fixed replays match** using an isolated original runtime after the system updated. Fifteen archive/command checks pass. The user chose local storage for now; off-device storage is deferred and hosted verification remains pending publication. This recovery check adds no independent samples and makes no gameplay, migration or deployment change.

The [initial policy review](Balance%20Harness/Idle-Policy-Review.md) identified the original control's starter and encounter-count gaps. The First Hunt extension addresses those selections while retaining fixed possible Armor/Jewelry Chest outcomes and untrained Essences as explicit conditions. Goals outside the selected Blood Grove and Crystal Creek starter policies remain draft. The first hosted CI run passed, and a disposable local character completed the prepared level-5-to-10 onboarding/access checks; broader progression-state review remains open in the [delivery plan](Balance%20Harness/Balance-Harness-Plan-With-Benchmarking.md). This supports Step 10, but does not yet certify the four Beta build directions, the level-1-to-30 journey, dungeon carryover or acquisition pacing. Human journey validation and the wider chapter/build roadmap remain necessary.
...
| First several days | Progress through five-level area bands, complete side quests that introduce a second Essence, ascension, focus, dungeons, crafting, gathering, Prophecies, and Colosseum. | Allocate newly unlocked Essence slots, select a Creature Focus, choose armor/weapon profiles, and decide which side systems to enter. | **Excitement:** two to four Essence slots begin to create combinations. **Frustration:** system introductions compete rather than form a sequence; random area enemy pools favor a general build; the player sees advanced collection and currency concepts before a stable build identity exists. Evidence: `EssenceSlotUnlockService`, `regions.json`, quest definitions, and Soul Archive screens. |
