...
def validate_admission(request_path, harness, pin):
    ...
    require(digest(Path(__file__)) == files['run-affinity-creation-recognition.py']
...
def launch(request_path, harness, dotnet, admission_pin=None):
    ...
    try:
        ...
        process = run([dotnet, str(harness), 'tower-affinity-creation-recognition-run', str(output)],
        ...
        audit_process = run([dotnet, str(harness), 'tower-affinity-creation-recognition-audit', str(output)],
        ...
        publication = run([dotnet, str(harness), 'tower-affinity-creation-recognition-publication-check', str(output)],
