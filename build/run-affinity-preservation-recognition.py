"""Single owned launch of the frozen twelve-root recognition diagnostic. Never retries or prepares inputs.
...
CUMULATIVE_SECONDS, CUMULATIVE_BYTES = 11400, 6979321856
PRIOR_SECONDS, PRIOR_BYTES = 600, 536870912
NATIVE_SECONDS, NATIVE_BYTES = 9000, 5905580032
AUDIT_SECONDS, AUDIT_BYTES = 1800, 536870912
...
def validate_request(q):
    require(q['version'] == VERSION and q.get('priorSeconds') in (600,1200)
            and q.get('priorBytes') == q['priorSeconds']//600*PRIOR_BYTES
...
def validate_admission(request_path, harness, pin):
    ...
    require(digest(Path(__file__)) == files['run-affinity-preservation-recognition.py']
...
def launch(request_path, harness, dotnet, admission_pin=None):
    ...
    try:
        ...
        process = run([dotnet, str(harness), 'tower-affinity-preservation-recognition-run', str(output)],
        ...
        audit_process = run([dotnet, str(harness), 'tower-affinity-preservation-recognition-audit', str(output)],
        ...
        publication = run([dotnet, str(harness), 'tower-affinity-preservation-recognition-publication-check', str(output)],
