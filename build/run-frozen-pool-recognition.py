...
def validate_admission(request_path, harness, pin):
    ...
    require(digest(Path(__file__)) == files['run-frozen-pool-recognition.py']
...
def launch(request_path, harness, dotnet, admission_pin=None):
    ...
    try:
        ...
        process = run([dotnet, str(harness), 'tower-frozen-pool-recognition-run', str(output)],
        ...
        audit_process = run([dotnet, str(harness), 'tower-frozen-pool-recognition-audit', str(output)],
        ...
        publication = run([dotnet, str(harness), 'tower-frozen-pool-recognition-publication-check', str(output)],
