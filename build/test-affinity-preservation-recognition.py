...
audit = module('confirmation_audit',ROOT/'Balance Harness/analysis/audit-affinity-preservation-recognition.py')
launcher = module('confirmation_launcher',ROOT/'build/run-affinity-preservation-recognition.py')


class ProtocolTests(unittest.TestCase):
    plan = audit.read(ROOT/'Balance Harness/Tower-Affinity-Preservation-Recognition-Plan.json')

    def rows(self):
        rows = []
        for r, root in enumerate(self.plan['roots']):
            for i, team in enumerate(root['teams']):
                count = [70,80,100][i] if i < 3 else 140
                rows.append(dict(partyId=audit.cell_id(len(rows),team),trials=[dict(seed=r*256+n,
                    outcome='Victory' if n < count else 'Draw') for n in range(256)]))
        return rows

    def test_plan_and_weighted_estimands(self):
        self.assertEqual(audit.PLAN,audit.sha(ROOT/'Balance Harness/Tower-Affinity-Preservation-Recognition-Plan.json'))
        result = audit.endpoint(self.plan,self.rows(),list(range(3072)))
        self.assertEqual((145,327,48,176),tuple(len(result[k]) for k in ('rates','contrasts','strata','unmeasured')))
        self.assertTrue(all(t['independentOutcome'] is None for t in result['unmeasured']))
        self.assertEqual([],result['populations'])
        cells = result['pairedPool']['cells']
        self.assertEqual(321,len(cells)); self.assertEqual(145,sum(c['measured'] for c in cells))
        for root in result['pairedPool']['roots']:
            estimates=root['estimates']
            for arm in ('control','candidate'):
                self.assertAlmostEqual(40/256,estimates[arm]['mean'])
                self.assertAlmostEqual(0,estimates[arm]['samplingVariance'])
                self.assertAlmostEqual((40/256)*(1-40/256)/255,estimates[arm]['combatVariance'])
                self.assertAlmostEqual(6/256,estimates[arm+'Nominees']['mean'])
            self.assertAlmostEqual(0,estimates['difference']['mean'])
            self.assertAlmostEqual(0,estimates['difference']['combatVariance'])
        ...
        self.assertAlmostEqual(NormalDist().inv_cdf(1-.025/799),audit.Z,7)

    def test_exact_variable_root_boundaries(self):
        expected=[r for r,size in enumerate(audit.ROOT_SIZES) for _ in range(size)]
        self.assertEqual(expected,[audit.root_index(i) for i in range(145)])
        for bad in (-1,145,True,1.5):
            with self.assertRaises(ValueError): audit.root_index(bad)

    def test_sampling_variance_has_finite_population_correction(self):
        root=dict(teams=[dict(stratum='remaining')]*2,strata=dict(remaining=dict(populationCount=10,sampleCount=2)))
        gains=[[.1]*256,[.3]*256]
        self.assertAlmostEqual(.8,audit.sampling_covariance(root,gains,[1,1],[1,1]))
        self.assertAlmostEqual(-.8,audit.sampling_covariance(root,gains,[1,1],[-1,-1]))
        root['strata']['remaining']['populationCount']=2
        self.assertEqual(0,audit.sampling_covariance(root,gains,[1,1],[1,1]))

    def test_shared_cells_cancel_and_covariance_identity_holds(self):
        rows=self.rows()
        teams=[t for root in self.plan['roots'] for t in root['teams']]
        for i,(row,team) in enumerate(zip(rows,teams)):
            count=65+(i%13)*9+audit.root_index(i)
            for n,trial in enumerate(row['trials']): trial['outcome']='Victory' if n<count else 'Defeat'
        result=audit.endpoint(self.plan,rows,list(range(3072)))
        for summary in [r['estimates'] for r in result['pairedPool']['roots']]+[result['pairedPool']['allRoots']]:
            for component in ('sampling','combat'):
                self.assertAlmostEqual(summary['difference'][component+'Variance'],summary['candidate'][component+'Variance']+
                    summary['control'][component+'Variance']-2*summary[component+'Covariance'])
        for root in self.plan['roots']:
            gains=[[float(n < 30+i) if t['membership']=='shared' else 0. for n in range(256)] for i,t in enumerate(root['teams'])]
            summary=audit.root_estimates(root,gains)
            self.assertEqual(0,summary['difference']['mean'])
            self.assertEqual(0,summary['difference']['samplingVariance'])
            self.assertEqual(0,summary['difference']['combatVariance'])
            self.assertGreater(summary['combatCovariance'],0)

    def test_unknown_outcomes_are_rejected_if_imputed(self):
        plan=copy.deepcopy(self.plan); plan['roots'][0]['unmeasured'][0]['independentOutcome']=0
        with self.assertRaisesRegex(ValueError,'Imputed'): audit.endpoint(plan,self.rows(),list(range(3072)))

    def test_receipt_preserves_verified_native_number_tokens(self):
        raw = '{"variance": 1.2345678901234567E-30, "zero": 0, "integerDouble": 1, "negative": -0}'
        audited = dict(status='Passed',result=json.loads(raw),newFights=0,newValues=0)
        with tempfile.TemporaryDirectory() as folder:
            path=Path(folder)/'audit.json'
            audit.write_receipt(path,audited,raw)
            self.assertEqual(audited,audit.read(path))
            ...
            with self.assertRaisesRegex(ValueError,'changed after'):
                audit.write_receipt(changed,audited,raw.replace('1.2345678901234567E-30','2.0'))
    ...
    def test_pairing_identity_faults_and_omissions_fail(self):
        ...
        for change in ('rows','cross-root','missing','duplicate','fault','identity'):
            ...
            if change == 'cross-root': rows[13]['trials'] = rows[0]['trials']
            ...
            if change == 'identity': rows[13]['partyId'] = rows[0]['partyId']
    ...
    def test_distinct_profile_rejects_legacy_requests_and_plan_pins(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            q=dict(version=legacy.VERSION,priorSeconds=600,priorBytes=536870912,maximumSeconds=11400,maximumBytes=6979321856,
    ...
    def test_admission_requires_external_pin_exact_runtime_and_launcher(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            for name in ('run-affinity-preservation-recognition.py','bounded_windows_process.py'):
            ...
            (p/'run-affinity-preservation-recognition.py').write_text('changed')
...
class OwnerTests(unittest.TestCase):
    ...
    def exercise(self,failure=None):
        with tempfile.TemporaryDirectory() as folder:
            ...
            auditor = ROOT/'Balance Harness/analysis/audit-affinity-preservation-recognition.py'
            q = dict(version=launcher.VERSION,outputRoot=str(output),registryRoot=str(registry),maximumSeconds=11400,maximumBytes=6979321856,
            ...
            def run(command,cwd,log,deadline,**kwargs):
                ...
                if len(calls) == 1:
                    launcher.write(output/'native-receipt.json',dict(version=launcher.VERSION,status='Verified',requestFileHash=launcher.digest(output/'request.json'),newAuditFights=0,fights=37120))
...
class SavedRowsTests(unittest.TestCase):
    ...
    def test_full_native_fixture_recounts_independently_and_rejects_report_reordering(self):
        ...
        self.assertEqual('Passed',result['status']); self.assertEqual(327,len(result['result']['contrasts']))
