...
def main(args):
    launcher=module('recognition_owned_launcher',ROOT/'build/run-affinity-preservation-recognition.py')
    audit=module('recognition_owned_auditor',ROOT/'Balance Harness/analysis/audit-affinity-preservation-recognition.py')
    ...
    launcher.require(root.name.startswith('tower-preservation-recognition-owned-fixture-') and not root.exists(),'Require a new explicitly synthetic output')
    ...
    def process(command,log):
        ...
        return value
    context_process=process(['dotnet',str(harness),'tower-affinity-preservation-recognition-context',str(ROOT/'LL/src/API/API.LL')],root/'context.log')
    ...
    q['recognition']=dict(planPath=str(ROOT/'Balance Harness/Tower-Affinity-Preservation-Recognition-Plan.json'),
        auditorPath=str(ROOT/'Balance Harness/analysis/audit-affinity-preservation-recognition.py'),auditorHash=audit.sha(ROOT/'Balance Harness/analysis/audit-affinity-preservation-recognition.py'))
    ...
    q.update(priorSeconds=1200,priorBytes=1073741824,maximumSeconds=12000,maximumBytes=7516192768)
    launcher.write(root/'request.json',q)
    launcher.write(root/'fixture.json',dict(version='synthetic-fixed-family-confirmation-fixture-v1',mode='preservation-recognition-owned',request=q,
        settings=audit.read(source/'study/scope.json')['settings']))
    commands=[]
    def routed(command,*positional,**kwargs):
        if len(command)>2 and command[2].startswith('tower-affinity-preservation-recognition-'):
            action=command[2][len('tower-affinity-preservation-recognition-'):]
            command=['dotnet',str(host),'preservation-recognition-fixture-'+action,str(root/'fixture.json')]
    ...
    receipt=process(['dotnet',str(host),'preservation-recognition-fixture-verify',str(root/'fixture.json')],root/'verification.log')
    ...
    launcher.require(len(commands)==4 and len(result['contrasts'])==327 and result['decision']=='CompleteDiagnosticOnly','Incomplete end-to-end fixture')
    summary=dict(status='OwnedRecognitionFixturePassed',literalReports=37120,actualCombat=0,productionEntropyDraws=0,
