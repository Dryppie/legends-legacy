...
def main(args):
    launcher=module('recognition_owned_launcher',ROOT/'build/run-frozen-pool-recognition.py')
    audit=module('recognition_owned_auditor',ROOT/'Balance Harness/analysis/audit-frozen-pool-recognition.py')
    ...
    launcher.require(root.name.startswith('tower-recognition-owned-fixture-') and not root.exists(),'Require a new explicitly synthetic output')
    ...
    def process(command,log):
        ...
        return value
    context_process=process(['dotnet',str(harness),'tower-frozen-pool-recognition-context',str(ROOT/'LL/src/API/API.LL')],root/'context.log')
    ...
    q['recognition']=dict(planPath=str(ROOT/'Balance Harness/Tower-Frozen-Pool-Recognition-Plan.json'),
        auditorPath=str(ROOT/'Balance Harness/analysis/audit-frozen-pool-recognition.py'),auditorHash=audit.sha(ROOT/'Balance Harness/analysis/audit-frozen-pool-recognition.py'))
    ...
    launcher.write(root/'fixture.json',dict(version='synthetic-fixed-family-confirmation-fixture-v1',mode='recognition-owned',request=q,
        settings=audit.read(source/'study/scope.json')['settings']))
    commands=[]
    def routed(command,*positional,**kwargs):
        if len(command)>2 and command[2].startswith('tower-frozen-pool-recognition-'):
            action=command[2][len('tower-frozen-pool-recognition-'):]
            command=['dotnet',str(host),'recognition-fixture-'+action,str(root/'fixture.json')]
    ...
    receipt=process(['dotnet',str(host),'recognition-fixture-verify',str(root/'fixture.json')],root/'verification.log')
