...
### Trigger/effect targeting optimization (retained 2026-08-18)

The measured trigger/effect transient layer was replaced without changing the public combat model:

- Instance method-group delegates passed into every trigger execution were replaced by a stack-only usage tracker.
- Trigger effects and primary-target discovery now use indexed traversal instead of interface enumerators and LINQ iterator chains.
- Effect target sets are selected imperatively into `ArrayPool<RuntimeCombatant>` snapshots and returned in `finally` blocks.
- First-on-tie behavior, combatant order, summon snapshot semantics, and random-call order are preserved. Random multi-target selection still assigns one `_random.Next()` value to every eligible candidate and performs the equivalent stable top-N selection.

All three retained Debug runs processed 8,641 actions and produced the accepted fingerprint. The comparison below uses the nearest fingerprinted pre-change run and the first three-run post-change result. Duration and CPU are shown for completeness, but the machine subsequently shifted to a slower load state, so allocation is the reliable retention signal and the duration change must not be treated as a stable throughput claim.

| Median measurement        | Fingerprinted before | Retained targeting | Observed change |
| ------------------------- | -------------------: | -----------------: | --------------: |
| Server resolve            |             12.672 s |           10.512 s |          -17.0% |
| Simulation                |             11.541 s |            9.508 s |          -17.6% |
| Simulation allocation     |            1.543 GiB |          1.252 GiB |      **-18.8%** |
| Request-window allocation |            1.658 GiB |          1.367 GiB |      **-17.5%** |
| CPU                       |             15.375 s |           13.000 s |          -15.4% |
| GC pause                  |             183.7 ms |           131.2 ms |          -28.6% |
| Gen 0 collections         |                  134 |                 77 |          -42.5% |

The earlier fixed-fixture median was 10.496 seconds server-side, essentially equal to the retained 10.512-second result. This supports a no-regression conclusion, not a claim that the targeting patch alone improved wall time by 17%. The before report is under `TestResults/idle-combat-benchmark/20260818-104616/`; the retained report is under `TestResults/idle-combat-benchmark/20260818-111754/`.

A new verbose allocation trace from the retained implementation sampled 1.403 GiB. The removed `CompiledEffect`/`CompiledTrigger` enumerators, target-selection display classes, and per-trigger `Func`/`Action` delegates no longer appear among its leaders. `CombatEvent` is now the largest sampled type at 579.2 MiB; 302.8 MiB of those samples were attributed to the record clone generated by the two target-specific condition checks. The trace is under `TestResults/idle-combat-benchmark/20260818-112148/`.

An explicit-target condition prototype removed those two record clones while keeping `CombatEvent` as a reference record. It matched the correctness fingerprint in all three runs, but an immediate revert-and-control comparison showed that it did not pass the retention gate:

| Median measurement        | Clone-free prototype | Reverted control | Prototype change |
| ------------------------- | -------------------: | ---------------: | ---------------: |
| Server resolve            |             13.664 s |         13.488 s |            +1.3% |
| Simulation                |             12.482 s |         12.318 s |            +1.3% |
| Simulation allocation     |            1.179 GiB |        1.253 GiB |            -5.9% |
| Request-window allocation |            1.296 GiB |        1.317 GiB |            -1.6% |
| CPU                       |             16.266 s |         16.391 s |            -0.8% |
| GC pause                  |             149.1 ms |         158.0 ms |            -5.6% |

The prototype traded a small request-window allocation reduction for a small wall-time regression and was reverted. This also confirms that the much larger difference from the earlier 10.512-second run was environmental load, not the prototype itself. The rejected report is under `TestResults/idle-combat-benchmark/20260818-112938/`; the reverted control is under `TestResults/idle-combat-benchmark/20260818-113232/`. Do not repeat event-representation work without a design that addresses the previously measured JIT/code-shape cost.

### Post-targeting CPU profile and barrier prototype (2026-08-18)

A fresh sampled-thread-time trace was captured from the retained targeting implementation. The request processed 8,641 actions, matched the accepted correctness fingerprint, and took 12.725 seconds under profiling. Isolating stack intervals below `FastCombatEngine.Run` produced 10.914 seconds of engine samples.
...
The prototype matched the correctness fingerprint in all three runs but failed the immediate A/B retention gate:

| Median measurement        | Direct-scan prototype | Reverted control | Prototype change |
| ------------------------- | --------------------: | ---------------: | ---------------: |
| Server resolve            |              13.856 s |         12.272 s |           +12.9% |
| Simulation                |              12.659 s |         10.508 s |           +20.5% |
| Simulation allocation     |             1.253 GiB |        1.160 GiB |            +8.0% |
| Request-window allocation |             1.369 GiB |        1.369 GiB |             0.0% |
| CPU                       |              17.141 s |         15.875 s |            +8.0% |
...
Status: still deferred after remeasurement. Neither allocation profile identified encounter-record retention as a leading source of total allocation, and the retained targeting trace instead points to engine event and runtime-state construction. Do not implement this phase without heap-retention or working-set evidence that the retained batch graphs are material.

This remains a bounded fallback because it does not require parallel execution or approximate outcomes.
...
- [x] Remove the measured trigger/effect iterator, delegate, target-array, and primary-target allocations.
- [x] Retain the targeting optimization after the three-run fingerprint and allocation gate.
- [x] Capture a fresh allocation trace from the retained targeting implementation.
- [x] Measure and reject the explicit-target event-clone prototype.
- [x] Capture and rank a post-targeting CPU trace.
- [x] Measure and reject the barrier direct-scan prototype.
- [x] Add characterization coverage for barrier application order and stable ties.
- [ ] Reassess compact within-batch facts only if heap-retention evidence makes Phase 2 material.
...
- [ ] Decide whether the remaining exact-replay cost justifies a compact runtime-state redesign.
