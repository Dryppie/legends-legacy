...
{
    private static async Task<IncumbentTieStudy> RunStudy(IncumbentTieRequest q, TowerBossDiscoveryDefinition template,
        IncumbentTieReservation allocation, Action check, CancellationToken ct)
    {
        ...
        using (var attempts = new TowerPracticalSearch.Attempts(P(q, "attempts.jsonl"), Policy(q.Version).MaximumFights, check))
        {
            var study = await Execute(template, allocation, mechanics, async (arm, stage, scenario, seed, token) => {
                ...
                Require(archive.CacheHits == 0, "Unexpected cross-restart or selector cache reuse."); return value;
            }, Save, attempts.Event, () => TowerSelectionDiagnostic.LiveAttemptHash(P(q, "attempts.jsonl")), ct);
            Require(attempts.Started == attempts.Completed && attempts.Completed == Assess(study, allocation, template.ExcludedCombatSeeds.Count).Fights,
    ...
    internal static void VerifyAttempts(string path, IncumbentTieStudy study, int fights)
    {
    ...
    internal static async Task<IncumbentTieResult> VerifyStudy(IncumbentTieRequest q, CancellationToken ct,
        Func<LoadoutScope, IReadOnlyList<LoadoutTrial>, (BossGenerationMechanics Mechanics, TowerBossDiscoveryRun.Battle Battle)>? fixture = null)
    {
        ...
        var saved = TowerContractJson.Read<IncumbentTieStudy>(Path.Combine(output, "study.json"));
        var result = Assess(saved, allocation, template.ExcludedCombatSeeds.Count);
