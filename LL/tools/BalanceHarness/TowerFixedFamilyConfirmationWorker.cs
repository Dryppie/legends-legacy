...
public static partial class TowerFixedFamilyConfirmation
{
    internal static async Task<int> WorkerWithOperation(string output,
        Func<TowerFixedFamilyRequest, TowerPracticalLaunch, CancellationToken, Task<TowerFixedFamilyResult>> operation, CancellationToken ct)
    {
        var q = TowerContractJson.Read<TowerFixedFamilyRequest>(Path.Combine(output, "request.json")); ValidateRequest(q);
        ...
        var remaining = launch.Deadline-DateTimeOffset.UtcNow-TimeSpan.FromSeconds(CloseoutSeconds);
    ...
    private static ProcessStartInfo NativeWorker(string output)
    {
        ...
        foreach (var s in new[] { typeof(TowerFixedFamilyConfirmation).Assembly.Location, "tower-fixed-family-confirmation-worker", output }) start.ArgumentList.Add(s);
        return start;
    }

    public static Task<TowerFixedFamilyResult> Run(TowerFixedFamilyRequest q, CancellationToken ct = default) => RunWithWorker(q, NativeWorker, ct);

    internal static async Task<TowerFixedFamilyResult> RunWithWorker(TowerFixedFamilyRequest request,
        Func<string, ProcessStartInfo> workerStart, CancellationToken ct = default, Action<string>? boundary = null)
    {
        ct.ThrowIfCancellationRequested(); var q = Copy(request); Require(q.Version == Version, "Use the owned three-reference launcher."); ValidateRequest(q, true);
        ...
        stop.CancelAfter(TimeSpan.FromSeconds(q.MaximumSeconds-q.PriorSeconds-CloseoutSeconds)); Process? child = null;
        try
        {
        ...
        catch (Exception error)
        {
            ...
            var result = new TowerFixedFamilyResult(Version, stop.IsCancellationRequested ? "Cancelled" : "Invalid", "Failed", "IncompleteEvidence",
                "Hold", [], [], [], [], [], [], "NotAssessed", SamplingAssumption, null, null, error.Message);
    ...
    internal static TowerFixedFamilyResult Publish(TowerFixedFamilyRequest q, TowerPracticalLaunch launch, Stopwatch clock, CancellationToken ct, Action<string>? boundary = null)
    {
        ...
        var study = TowerContractJson.Read<TowerFixedFamilyStudy>(P(q, "study/study.json")); var result = Assess(study, HarnessJson.FileHash(P(q, "study/files.json")));
        ...
        var final = new TowerFixedFamilyCloseout(Version, launch.RequestHash, HarnessJson.FileHash(P(q, "files.json")), clock.Elapsed.TotalSeconds, 0);
        for (var i = 0; i < 10; i++)
        {
    ...
    public static Task<TowerFixedFamilyResult> Verify(string output, CancellationToken ct = default)
        => VerifyPublication(output, t => VerifyStudy(ReadArchiveRequest(output), t), ct);
    internal static TowerFixedFamilyRequest ReadArchiveRequest(string output)
        => TowerContractJson.Read<TowerFixedFamilyRequest>(Path.Combine(output, "request.json")) with { ArchiveRoot = Path.GetFullPath(output) };

    internal static async Task<TowerFixedFamilyResult> VerifyPublication(string output, Func<CancellationToken, Task<TowerFixedFamilyStudy>> verifyStudy, CancellationToken ct = default)
    {
        ...
        var q = ReadArchiveRequest(output); ValidateRequest(q); var final = TowerContractJson.Read<TowerFixedFamilyCloseout>(P(q, "closeout.json"));
        ...
        foreach (var name in PhaseNames)
        {
            ...
            Require(last < launch.Deadline.AddSeconds(-CloseoutSeconds), "Phase outside cumulative deadline.");
            if (name == "audit")
            {
        ...
        CheckEnclosingHeadroom(q, launch, TowerContractJson.Read<TowerDiagnosticPhase>(P(q, "phase.json")),
            launch.StartedAt.AddSeconds(final.MeasuredSeconds), final.RetainedBytes);
    ...
    {
        ct.ThrowIfCancellationRequested(); if (args is ["tower-fixed-family-confirmation-worker", var output]) return await Worker(output, ct);
        object result = args switch {
            ["tower-fixed-family-confirmation-check", var path] => await Check(TowerContractJson.Read<TowerFixedFamilyRequest>(path), ct),
            ["tower-fixed-family-confirmation-run", var path] => await Run(TowerContractJson.Read<TowerFixedFamilyRequest>(path), ct),
            ["tower-fixed-family-confirmation-verify", var path] => await Verify(path, ct),
            _ => throw new InvalidDataException("Use tower-fixed-family-confirmation-check|run <request.json> or tower-fixed-family-confirmation-verify <completed-output>. No retry, replay or resume.") };
        Console.WriteLine(JsonSerializer.Serialize(result, HarnessJson.Options)); return result is TowerFixedFamilyResult r && r.IntegrityStatus != "Verified" ? 2 : 0;
    }
}

internal sealed record TowerFixedFamilyCloseout(string Version, string RequestHash, string FilesHash, double MeasuredSeconds, long RetainedBytes);
