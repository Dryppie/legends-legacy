// This fixture uses the sealed comparison model compiled by its isolated protocol.
#if TOWER_CORE_PORTFOLIO_COMPARISON_FIXTURE
using BalanceHarness;
using Domain.Models.Combat;
namespace EssenceSystem.Tests;

public sealed class BalanceHarnessCorePortfolioComparisonTests : IDisposable
{
    ...
    static ComparisonMember[] Nominations() {
        ...
        return [new("a",a,["baseline-rank-1"]),new("b",b,["baseline-rank-2"]),new("b",b,["team-core-portfolio-rank-1"]),new("a",a,["team-core-portfolio-rank-2"])];
    }
    ...
    [Fact] public void Exact_native_and_global_costs_share_only_policy_independent_inputs() {
        var a=Definition();var b=Definition("team-core-portfolio");ComparisonModel.Shared(a,b);
        HarnessJson.WriteNew(Path.Combine(AppContext.BaseDirectory,"fixture-baseline-definition.json"),a);
        HarnessJson.WriteNew(Path.Combine(AppContext.BaseDirectory,"fixture-team-core-portfolio-definition.json"),b);
    ...
    [Fact] public async Task Both_policy_nominations_are_independent_and_incomplete_discovery_stops() {
        foreach(var policy in ComparisonModel.Policies) {
            ...
            var prior=HarnessJson.Read<BossGenerationResult>(Path.Combine(AppContext.BaseDirectory,policy=="baseline"?"team.json":"portfolio.json"));
            foreach(var pair in g.Arms.Single().Proposals.Zip(prior.Arms.Single().Proposals)) {
                Assert.Equal(HarnessJson.Hash(pair.Second.Party),HarnessJson.Hash(pair.First.Party));
                Assert.Equal(HarnessJson.Hash(pair.Second.JointStructural),HarnessJson.Hash(pair.First.JointStructural));
            }
    ...
    [Fact] public void Shared_winners_and_controls_merge_without_refilling_or_losing_origins() {
        ...
        Assert.Equal("team-core-portfolio-finalist-minus-baseline-finalist",q.Primary);Assert.Equal(new TowerSearchBenchmarkPair(32,0,0,0,0,0),q.Differences[q.Primary]);
    }
    ...
    [Fact] public void Interrupted_attempt_is_durable_and_consumes_the_cap() {
        var root=Path.Combine(Path.GetTempPath(),"team-core-portfolio-comparison-"+Guid.NewGuid().ToString("N"));Directory.CreateDirectory(root);var path=Path.Combine(root,"attempts.bin");
