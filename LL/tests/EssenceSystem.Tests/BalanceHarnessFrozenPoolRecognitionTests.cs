...
public sealed class BalanceHarnessFrozenPoolRecognitionTests(ITestOutputHelper output)
{
    private static string Repository()
    {
        for (var p = Directory.GetCurrentDirectory(); p is not null; p = Path.GetDirectoryName(p))
            if (File.Exists(Path.Combine(p, "Balance Harness/Tower-Frozen-Pool-Recognition-Plan.json"))) return p;
        throw new IOException("Run through the repository test wrapper.");
    }
    private static string Plan => Path.Combine(Repository(), "Balance Harness/Tower-Frozen-Pool-Recognition-Plan.json");
    internal static TowerFixedFamilyDefinition Definition()
    {
        var p = C.RecognitionPlan(Plan); var runtime = p.GetProperty("capturedRuntime");
        return new(C.RecognitionVersion,C.RecognitionTeams(p),runtime.GetProperty("contentHashes").Deserialize<Dictionary<string,string>>(HarnessJson.Options)!,
    ...
    {
        var d = Definition(); C.ValidateDefinition(d); Assert.Equal(C.RecognitionTeamsHash,HarnessJson.Hash(d.Teams));
    ...
    public void Full_batch_reserves_unused_tail_and_short_batch_never_becomes_complete()
    {
        ...
        var p = C.Classify(bytes,[-987],new string('a',64),C.RecognitionVersion);
        ...
        Assert.Single(C.Classify(new byte[24576],[-987],new string('a',64),C.RecognitionVersion).Panel);
    }
    private static (TowerFixedFamilyRequest Q,TowerFixedFamilyInputs Input) Fixture(string root)
    {
        ...
        { Recognition = new(Plan,Path.Combine(Repository(),"Balance Harness/analysis/audit-frozen-pool-recognition.py"),
            HarnessJson.FileHash(Path.Combine(Repository(),"Balance Harness/analysis/audit-frozen-pool-recognition.py"))) };
    ...
    {
        var root = Path.Combine(Path.GetTempPath(),"tower-recognition-envelope-"+Guid.NewGuid().ToString("N"));
        try
        {
    ...
    public async Task Full_27648_report_archive_reconstructs_without_combat_and_is_retained_for_python()
    {
        ...
        var saved = Environment.GetEnvironmentVariable("FROZEN_POOL_RECOGNITION_FIXTURE");
        var root = saved ?? Path.Combine(Path.GetTempPath(),"tower-recognition-fixture-"+Guid.NewGuid().ToString("N"));
