...
public sealed class BalanceHarnessAffinityPreservationRecognitionTests(ITestOutputHelper output)
{
    private static string Repository()
    {
        for (var p = Directory.GetCurrentDirectory(); p is not null; p = Path.GetDirectoryName(p))
            if (File.Exists(Path.Combine(p, "Balance Harness/Tower-Affinity-Preservation-Recognition-Plan.json"))) return p;
        throw new IOException("Run through the repository test wrapper.");
    }
    private static string Plan => Path.Combine(Repository(), "Balance Harness/Tower-Affinity-Preservation-Recognition-Plan.json");
    internal static TowerFixedFamilyDefinition Definition()
    {
        var p = C.RecognitionPlan(Plan,C.PreservationRecognitionVersion); var runtime = p.GetProperty("capturedRuntime");
        return new(C.PreservationRecognitionVersion,C.RecognitionTeams(p),runtime.GetProperty("contentHashes").Deserialize<Dictionary<string,string>>(HarnessJson.Options)!,
            runtime.GetProperty("settingsHash").GetString()!,HarnessJson.Hash(ExecutionIdentity.Current()),[-987]);
    }
    private static int LiteralWins(TowerFixedFamilyDefinition d,int i)
    {
        var root = C.RecognitionRoot(d.Version,i); var local = i-C.PreservationRootSizes.Take(root).Sum();
        return local < 3 ? new[] {70,80,100}[local] : 140;
    }
    private static TowerFixedFamilyStudy Study()
    {
        ...
        return new(d.Version,f,d.Teams.Select((t,i) => new TowerDiagnosticCell(C.CellId(d,i),
            Enumerable.Range(0,256).Select(n => new TowerBalanceTrial(C.RecognitionRoot(d.Version,i)*256+n,
                n < LiteralWins(d,i) ? BattleOutcome.Victory : BattleOutcome.Draw)).ToArray())).ToArray());
    }
    [Fact]
    public void Root_panels_and_recipes_are_exact_disjoint_and_never_deduplicated()
    {
        var d = Definition(); C.ValidateDefinition(d); Assert.Equal(C.PreservationRecognitionTeamsHash,HarnessJson.Hash(d.Teams));
        var chunks = C.Chunks(d,Enumerable.Range(0,3072).ToArray()); Assert.Equal(145,chunks.Count);
        Assert.Equal(145,chunks.Select(c => c.PartyId).Distinct().Count());
        foreach (var chunk in chunks) Assert.Equal(Enumerable.Range(C.RecognitionRoot(d.Version,chunk.TeamOrdinal)*256,256),chunk.Scenario.Seeds);
    ...
    public void Weighted_diagnosis_preserves_nulls_and_cannot_promote_a_team()
    {
        ...
        Assert.Equal(145,r.Rates.Count); Assert.Equal(327,r.Contrasts.Count); Assert.Equal(48,r.Strata.Count); Assert.Equal(176,r.Unmeasured.Count);
        ...
        Assert.Equal(C.PreservationRecognitionVersion,r.Version);
        Assert.Empty(r.Populations); Assert.NotNull(r.PairedPool); Assert.Equal(321,r.PairedPool.Cells.Count);
        Assert.Equal(145,r.PairedPool.Cells.Count(c => c.Measured)); Assert.Equal(799,r.ApproximateWilsonFamily);
        foreach (var root in r.PairedPool.Roots) {
            Assert.Equal(40/256d,root.Estimates.Control.Mean,12); Assert.Equal(40/256d,root.Estimates.Candidate.Mean,12);
            Assert.Equal(0,root.Estimates.Difference.Mean,12); Assert.Equal(0,root.Estimates.Difference.CombatVariance,12);
            Assert.Equal(6/256d,root.Estimates.ControlNominees.Mean,12); Assert.Equal(6/256d,root.Estimates.CandidateNominees.Mean,12);
            Assert.Equal(0,root.Estimates.Control.SamplingVariance,12);
            Assert.Equal((40/256d)*(1-40/256d)/255,root.Estimates.Control.CombatVariance,12);
        }
        Assert.Equal(40/256d,r.PairedPool.AllRoots.Control.Mean,12);
        foreach (var bad in new[] {
            s with { Evidence = s.Evidence.Reverse().ToArray() },
            s with { Evidence = s.Evidence.Select((e,i) => i == 13 ? e with { Trials = s.Evidence[0].Trials } : e).ToArray() },
    ...
    public void Full_batch_reserves_unused_tail_and_short_batch_never_becomes_complete()
    {
        ...
        var p = C.Classify(bytes,[-987],new string('a',64),C.PreservationRecognitionVersion);
        ...
        Assert.Single(C.Classify(new byte[24576],[-987],new string('a',64),C.PreservationRecognitionVersion).Panel);
    }
    private static (TowerFixedFamilyRequest Q,TowerFixedFamilyInputs Input) Fixture(string root)
    {
        ...
        var q = new TowerFixedFamilyRequest(d.Version,content,source,HarnessJson.FileHash(source),root,Path.Combine(root,"result"),history,
            11400,6656L*1048576,new Dictionary<string,TowerDiagnosticPhaseLimit>(),600,512L*1048576,
            [new("Literal admission fixture",600,512L*1048576,prior,HarnessJson.FileHash(prior))])
        { Recognition = new(Plan,Path.Combine(Repository(),"Balance Harness/analysis/audit-affinity-preservation-recognition.py"),
            HarnessJson.FileHash(Path.Combine(Repository(),"Balance Harness/analysis/audit-affinity-preservation-recognition.py"))) };
    ...
    [Fact]
    public void Sampling_variance_and_negative_covariance_match_literal_finite_population()
    {
        var root = JsonSerializer.SerializeToElement(new {
            teams = new[] { new { stratum = "sample" }, new { stratum = "sample" } },
            strata = new { sample = new { populationCount = 10, sampleCount = 2 } }
        });
        double[][] gains = [Enumerable.Repeat(.1,256).ToArray(),Enumerable.Repeat(.3,256).ToArray()];
        Assert.Equal(.8,C.RecognitionSamplingCovariance(root,gains,[1,1],[1,1]),12);
        Assert.Equal(-.8,C.RecognitionSamplingCovariance(root,gains,[1,1],[-1,-1]),12);
        Assert.Equal(0,C.RecognitionSamplingCovariance(root,gains,[0,0],[0,0]),12);
        Assert.Throws<InvalidDataException>(() => C.RecognitionCovariance([1],[1]));
    }
    [Fact]
    public void Shared_only_effects_cancel_with_positive_cross_arm_covariance()
    {
        var s = Study(); var plan = C.RecognitionPlan(Plan,C.PreservationRecognitionVersion);
        var teams = plan.GetProperty("roots").EnumerateArray().SelectMany(r => r.GetProperty("teams").EnumerateArray()).ToArray();
        s = s with { Evidence = s.Evidence.Select((e,i) => e with { Trials = e.Trials.Select((t,n) => t with {
            Outcome = n < (teams[i].GetProperty("stratum").GetString() != "reference" && teams[i].GetProperty("membership").GetString() == "shared" ? 140+i%5 : 100)
                ? BattleOutcome.Victory : BattleOutcome.Defeat }).ToArray() }).ToArray() };
        var result = C.AssessRecognition(s,Plan,new string('c',64)).PairedPool!;
        foreach (var root in result.Roots) {
            var e = root.Estimates;
            Assert.Equal(e.Control.Mean,e.Candidate.Mean,12);
            Assert.Equal(0,e.Difference.Mean,12); Assert.Equal(0,e.Difference.CombatVariance,12); Assert.Equal(0,e.Difference.SamplingVariance,12);
            Assert.True(e.CombatCovariance > 0); Assert.Equal(e.Control.CombatVariance,e.CombatCovariance,12);
        }
    }
    [Fact]
    public void New_limits_and_last_root_cannot_be_substituted_into_legacy_profiles()
    {
        var limits = C.RecognitionLimits(C.PreservationRecognitionVersion);
        Assert.Equal(10800,limits.Seconds); Assert.Equal(6L*1073741824,limits.Bytes);
        Assert.Equal(37120,C.Policy(C.PreservationRecognitionVersion).Fights);
        Assert.Equal(11,C.RecognitionRoot(C.PreservationRecognitionVersion,144));
        Assert.Throws<InvalidDataException>(() => C.RecognitionRoot(C.PreservationRecognitionVersion,145));
        foreach (var old in new[] { C.RecognitionVersion,C.AffinityRecognitionVersion }) {
            Assert.Equal(7200,C.RecognitionLimits(old).Seconds); Assert.Equal(27648,C.Policy(old).Fights);
            Assert.Throws<InvalidDataException>(() => C.RecognitionPlan(Plan,old));
        }
    }
    [Fact]
    public void Both_profiles_reject_the_other_frozen_plan_and_family()
    {
        ...
        Assert.Throws<InvalidDataException>(() => C.RecognitionPlan(legacy,C.PreservationRecognitionVersion));
        Assert.Throws<InvalidDataException>(() => C.ValidateDefinition(Definition() with { Version = C.RecognitionVersion }));
        Assert.Throws<InvalidDataException>(() => C.ValidateDefinition(BalanceHarnessFrozenPoolRecognitionTests.Definition() with { Version = C.PreservationRecognitionVersion }));
    ...
    public async Task Command_profiles_reject_cross_version_requests_before_side_effects()
    {
        ...
        try
        {
            ...
            foreach (var expected in new[] {C.RecognitionVersion,C.PreservationRecognitionVersion})
            {
                var other = expected == C.RecognitionVersion ? C.PreservationRecognitionVersion : C.RecognitionVersion;
    ...
    {
        var root = Path.Combine(Path.GetTempPath(),"tower-preservation-recognition-envelope-"+Guid.NewGuid().ToString("N"));
        try
        {
            var (q,_) = Fixture(root); var receipt = Path.Combine(root,"failed-admission.json"); File.WriteAllText(receipt,"{}");
            var revised = q with { MaximumSeconds = 12000, MaximumBytes = 7168L*1048576, PriorSeconds = 1200, PriorBytes = 1024L*1048576,
            ...
            foreach (var bad in new[] { revised with { MaximumSeconds = 12001 }, revised with { PriorSeconds = 600 },
    ...
    [Fact]
    public async Task Full_37120_report_archive_reconstructs_without_combat_and_is_retained_for_python()
    {
        using var guard = new TowerPerformanceTrace(_ => throw new InvalidOperationException("Fixture entered combat.")).Activate();
        var saved = Environment.GetEnvironmentVariable("AFFINITY_PRESERVATION_RECOGNITION_FIXTURE");
        var root = saved ?? Path.Combine(Path.GetTempPath(),"tower-preservation-recognition-fixture-"+Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            using (var attempts = new TowerPracticalSearch.Attempts(C.P(q,"attempts.jsonl"),37120,() => {}))
            {
                study = await FixedFamilyFixtureHost.Study(q,freeze,panel,"complete",attempts.Event,() => {},default);
                Assert.Equal(37120,attempts.Started); Assert.Equal(37120,attempts.Completed);
            }
            ...
            output.WriteLine(JsonSerializer.Serialize(new { seconds = clock.Elapsed.TotalSeconds, bytes = TowerBulkCampaign.StorageBytes(root,default),
                literalReports = 37120, actualCombat = 0, productionEntropyDraws = 0, fixture = root }));
