...
public sealed class GameEventOutboxTests
{
    ...
    public void Consumer_registry_routes_new_activity_events_to_quests()
    {
        ...
        Assert.Equal(
            [GameEventOutboxConsumerNames.RealtimeRaid],
            registry.GetConsumers(GameEventTypes.RaidUpdated));
    }
    ...
    [Fact]
    public void Consumer_registry_routes_region_boss_announcements_to_durable_chat_delivery()
    {
        var registry = new GameEventOutboxConsumerRegistry();

        var consumer = Assert.Single(
            registry.GetConsumers(GameEventTypes.RegionBossChatAnnouncement));

        Assert.Equal(GameEventOutboxConsumerNames.RegionBossChat, consumer);
    }
    ...
    private sealed class RecordingAchievementService : IAchievementService
    {
        ...
        public Task<bool> UnlockTitleAsync(Guid accountId, Guid characterId, string titleKey, string? metadataJson, CancellationToken cancellationToken, bool announce = true) =>
