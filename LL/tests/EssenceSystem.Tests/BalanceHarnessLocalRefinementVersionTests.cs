...
public sealed class BalanceHarnessLocalRefinementVersionTests
{
    static F Local() => new(TowerSharedExecutable.Profile, Model.LocalVersion);
    static TowerBossDiscoveryDefinition[] Pair() {
        ...
        return Model.Policies.Select(p => Model.Definition(source, labels, p, Model.LocalVersion)).ToArray();
    }
    [Fact] public void Local_pair_changes_only_version_and_policy_with_identical_schedules() {
        var old = Runtime.Definitions(); var next = Pair(); Model.ValidatePair(next);
        Assert.Equal(TowerDiscoveryRefinementSearch.LocalVersion, next[1].Generation.PolicyVersion);
        for (var i = 0; i < 2; i++) {
            Assert.Equal(Model.LocalVersion, next[i].Id);
    ...
    [Fact] public void Local_preflight_requires_the_matching_verified_driver_receipt() {
        foreach (var policy in new[] { TowerDiscoveryRefinementSearch.Version, TowerDiscoveryRefinementSearch.NovelVersion, TowerDiscoveryRefinementSearch.LocalVersion }) {
            var receipt = JsonSerializer.SerializeToElement(new { status = "VerifiedRefinementComparisonDriver", reservations = 1, comparisonVersion = Model.LocalVersion, refinementPolicy = policy });
            if (policy == TowerDiscoveryRefinementSearch.LocalVersion) TowerRefinementComparisonPreflight.VerifyDriverReceipt(receipt, 1, Model.LocalVersion);
            else Assert.Throws<InvalidDataException>(() => TowerRefinementComparisonPreflight.VerifyDriverReceipt(receipt, 1, Model.LocalVersion));
        }
        var missing = JsonSerializer.SerializeToElement(new { status = "VerifiedRefinementComparisonDriver", reservations = 1 });
        Assert.Throws<InvalidDataException>(() => TowerRefinementComparisonPreflight.VerifyDriverReceipt(missing, 1, Model.LocalVersion));
    }
    [Fact] public void Changed_version_cannot_reuse_authorization_or_allocate_a_label() {
        var old = new F(TowerSharedExecutable.Profile, Model.NovelVersion); var inspected = false; var allocated = false;
        var changed = old.Request with { Preflight = old.Request.Preflight with { ComparisonVersion = Model.LocalVersion } };
    ...
    [Fact] public async Task Local_binding_and_reconstruction_preserve_charges_and_archive_integrity() {
        ...
        Assert.Equal(TowerDiscoveryComparisonGate.LocalVersion, HarnessJson.Read<TowerDiscoveryComparisonDecision>(f.P("run/discovery-gate.json")).Version);
    ...
    [Fact] public async Task Incomplete_local_discovery_preserves_charges_and_stops_selection() {
        ...
        Assert.Equal(TowerDiscoveryComparisonGate.LocalVersion, gate.Version); Assert.Equal("StoppedDiscovery", gate.Status);
    ...
    [Fact] public void Wrong_candidate_versions_and_nominations_are_rejected() {
        var definitions = Pair(); var wrong = definitions.ToArray(); wrong[1] = wrong[1] with { Generation = wrong[1].Generation with { PolicyVersion = TowerDiscoveryRefinementSearch.NovelVersion } };
        ...
        var nominations = new[] { TowerTeamCoverageSearch.Version, TowerDiscoveryRefinementSearch.NovelVersion }.SelectMany(policy => new[] { 1, 2 }
