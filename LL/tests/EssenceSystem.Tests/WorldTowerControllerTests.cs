...
public sealed class WorldTowerControllerTests
{
    ...
    public async Task EndpointsDispatchAuthenticatedCharacterAndRoutePayloads()
    {
        ...
        Assert.Equal(16, sender.Requests.Count);
        ...
        var combatResult = Assert.IsType<GetTowerAttemptCombatResultQuery>(sender.Requests[3]);
        Assert.Equal((characterId, attemptId), (combatResult.CharacterId, combatResult.AttemptId));
        Assert.IsType<GetTowerHallOfFameQuery>(sender.Requests[4]);
        Assert.Equal(
            characterId,
            Assert.IsType<GetPersonalTowerExpeditionsQuery>(sender.Requests[5]).CharacterId);
        ...
        var loadout = Assert.IsType<UpdateTowerRallyLoadoutCommand>(sender.Requests[11]);
        Assert.Equal((characterId, rallyId), (loadout.CharacterId, loadout.RallyId));
        var transfer = Assert.IsType<TransferTowerRallyLeadershipCommand>(sender.Requests[12]);
        Assert.Equal(
            (characterId, rallyId, newLeaderId),
            (transfer.CharacterId, transfer.RallyId, transfer.TargetCharacterId));
        var parties = Assert.IsType<UpdateTowerRallyPartiesCommand>(sender.Requests[13]);
        ...
        var start = Assert.IsType<StartTowerRallyCommand>(sender.Requests[14]);
        Assert.Equal((characterId, rallyId), (start.CharacterId, start.RallyId));

        var contribute = Assert.IsType<ContributeToTowerCommand>(sender.Requests[15]);
    ...
    public async Task PlaybackEndpointsDispatchAuthenticatedCharacter()
    {
        ...
        var bundle = Assert.IsType<GetTowerAttemptPlaybackBundleQuery>(sender.Requests[0]);
