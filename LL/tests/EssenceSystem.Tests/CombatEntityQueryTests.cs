...
public sealed class CombatEntityQueryTests
{
    [Fact]
    public async Task Combat_entities_lookup_compiles_without_multiple_collection_warnings()
    {
        ...
        // Compile the actual combat query, stopping before any database access.
        await Assert.ThrowsAsync<QueryCompiledException>(() =>
            repository.GetEntitiesByIdsForCombatAsync([Guid.NewGuid()], CancellationToken.None));
    }
