...
public sealed partial class BalanceHarnessFixedFamilyConfirmationTests
{
    [Theory]
    ...
    public async Task Owned_process_completes_negative_evidence_and_stops_at_phase_and_storage_caps(string mode)
    {
        ...
        if (mode == "negative")
        {
            var result = await C.VerifyPublication(q.OutputRoot, ct => FixedFamilyFixtureHost.Verify(q, ct));
    ...
    public async Task Owner_or_worker_death_keeps_blocking_pending_without_publication(string killed)
    {
        ...
        HarnessJson.WriteNew(fixture, new FixedFamilyFixture(FixedFamilyFixtureHost.Version, "entropy-pending-hang", q, Settings));
        var start = FixtureHost.Start("fixed-family-parent", fixture); start.RedirectStandardOutput = true; start.RedirectStandardError = true;
    ...
    private static async Task Ready(TowerFixedFamilyRequest q, Process? parent = null)
    {
    ...
    public async Task Cancellation_kills_started_attempt_and_registry_lease_prevents_a_second_writer()
    {
        ...
        var fixture = Path.Combine(root, "process.json"); HarnessJson.WriteNew(fixture, new FixedFamilyFixture(FixedFamilyFixtureHost.Version, "attempt-hang", q, Settings));
        using var stop = new CancellationTokenSource(); var operation = C.RunWithWorker(q, _ => FixtureHost.Start("fixed-family-worker", fixture), stop.Token);
        try
        {
    ...
    {
        var (q, _) = Input(); q = q with { MaximumSeconds = 3660, PriorSeconds = 1860, MaximumBytes = 2112L*1048576, PriorBytes = 1088L*1048576,
            PriorCharges = [q.PriorCharges[0] with { Seconds = 1860, Bytes = 1088L*1048576 }] };
        C.ValidateRequest(q); C.CheckEnvelope(q, 0, 0);
        Assert.Throws<InvalidDataException>(() => C.CheckEnvelope(q, 1740, 0));
        Assert.Throws<InvalidDataException>(() => C.CheckEnvelope(q, 0, 1008L*1048576+1));
        var launch = Launch(q); var phase = new TowerDiagnosticPhase("admission", launch.StartedAt, launch.StartedAt.AddSeconds(90), 0);
        HarnessJson.WriteNew(C.P(q, "phase.json"), phase);
        Assert.Throws<InvalidDataException>(() => C.CheckActivePhase(q, 64L*1048576+1));
        Assert.Throws<InvalidDataException>(() => C.ValidateRequest(q with { Phases = q.Phases.ToDictionary(p => p.Key,
            p => p.Key == "audit" ? p.Value with { Bytes = 257L*1048576 } : p.Value) }));
    }

    [Theory] [InlineData(true)] [InlineData(false)]
    public void Setup_headroom_cannot_borrow_unused_combat_or_audit_allowance(bool time)
    {
        var (q, _) = Input(); var launch = Launch(q);
        C.CheckEnclosingHeadroom(q,launch,null,launch.StartedAt,0);
        Assert.Throws<InvalidDataException>(() => C.CheckEnclosingHeadroom(q,launch,null,
            launch.StartedAt.AddSeconds(time ? 301 : 0), time ? 0 : 240L*1048576+1));
    }
    ...
    public async Task Every_publication_interruption_is_failed_even_if_result_or_closeout_was_written(string boundary)
    {
        ...
        var result = await C.RunWithWorker(q, _ => {
            ...
            var study = new TowerFixedFamilyStudy(C.Version, freeze, input.Definition.Teams.Select(t => new TowerDiagnosticCell(t.PartyId,
