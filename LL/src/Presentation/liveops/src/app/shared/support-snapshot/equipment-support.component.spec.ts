...
describe('EquipmentSupportComponent', () => {
  const empty = (): EquipmentSupportSnapshot => ({ rowLimit: 100, equipmentCount: 0, pendingRewardCount: 0,
    progressTruncated: false, items: [], pendingRewards: [], protection: [], ordinary: [], learnedStyles: [] });
  ...
  it('distinguishes saved paid investment, awarded ranks, and legacy equipment', async () => {
    await TestBed.configureTestingModule({ imports: [EquipmentSupportComponent] }).compileComponents();
    const fixture = TestBed.createComponent(EquipmentSupportComponent);
    const data = empty();
    ...
    data.items = [
      { instanceId: 'gear', itemBaseId: 'sword', name: 'Named Sword', locations: ['Equipped: MainHand'], progression: {
        ...
        sourceId: 'dungeon', awardId: 'run', baseSalvageScrap: 0, paidScrap: 20, paidCinders: 1000,
        investments: [{ operationId: 'paid-operation', rank: 3, scrap: 20, cinders: 1000 }],
      } },
    ...
    expect(text).toContain('20 paid Scrap');
    expect(text).toContain('Paid rank 3');
    expect(text).toContain('paid-operation');
