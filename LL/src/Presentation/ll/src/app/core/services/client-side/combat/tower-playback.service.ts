...
export class TowerPlaybackService {
  ...
  private materializeFrame(
    ...
    const entityStates = new Map<
      number,
      TowerPlaybackBundleFrame['entityStates'][number]
    >();
    const entityTotals = new Map<
      number,
      TowerPlaybackBundleFrame['entityTotals'][number]
    >();
    const abilityTotals = new Map<
      number,
      TowerPlaybackBundleFrame['abilityTotals'][number]
    >();
  ...
  private toCombatFrame(
    ...
    const stats = activeEntities.map((entity): EntityStats => {
      ...
      return {
        ...
        targetedAttacks: totals?.targetedAttacks ?? 0,
        attentionSharePercent: totals?.attentionSharePercent ?? 0,
