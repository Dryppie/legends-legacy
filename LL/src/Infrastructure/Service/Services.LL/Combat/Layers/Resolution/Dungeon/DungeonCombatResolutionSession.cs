...
public sealed class DungeonCombatResolutionSession : ICombatResolutionSession
{
    public DungeonCombatTemplateCatalog Catalog = null!;
    private readonly ICombatEngineExecutor _engineExecutor;
    private readonly ICombatEncounterResultFactory _resultFactory;

    public DungeonCombatResolutionSession(
        ...
        CombatParticipantSlot slot)
    {
        if (!Catalog.SourceEntitiesById.TryGetValue(slot.SourceEntityId, out var sourceEntity))
        {
            throw new InvalidOperationException(
                $"Friendly source entity '{slot.SourceEntityId}' was not found in dungeon resolution catalog.");
        }

        if (!Catalog.FriendlyTemplatesBySourceEntityId.TryGetValue(slot.SourceEntityId, out var template))
        {
            throw new InvalidOperationException(
                $"Friendly template for source entity '{slot.SourceEntityId}' was not found in dungeon resolution catalog.");
        }
        ...
        CombatParticipantSlot slot)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Hostile source entity '{slot.SourceEntityId}' was not found in dungeon resolution catalog.");
        }

        if (!Catalog.HostileTemplatesBySourceEntityId.TryGetValue(slot.SourceEntityId, out var template))
        {
            throw new InvalidOperationException(
                $"Hostile template for source entity '{slot.SourceEntityId}' was not found in dungeon resolution catalog.");
        }
