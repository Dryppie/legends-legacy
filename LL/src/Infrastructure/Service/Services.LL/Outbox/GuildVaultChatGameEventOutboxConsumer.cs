...
public sealed class GuildVaultChatGameEventOutboxConsumer(
    ...
    JsonSerializerOptions jsonOptions) : IGameEventOutboxConsumer
{
    ...
    public string Consumer => GameEventOutboxConsumerNames.GuildVaultChat;

    public bool CanHandle(string eventType) =>
        string.Equals(eventType, GameEventTypes.GuildVaultChatMessage, StringComparison.OrdinalIgnoreCase);
    ...
    public async Task HandleAsync(GameEventOutboxMessage message, CancellationToken cancellationToken)
    {
        ...
        {
            Content = JsonContent.Create(new GuildChatMessageRequest(
                payload.GuildId,
                ...
                payload.Equipment,
                payload.MessageId,
                payload.SentAt), options: jsonOptions)
        };
