...
public sealed class RegionBossChatGameEventOutboxConsumer(
    ...
    JsonSerializerOptions jsonOptions) : IGameEventOutboxConsumer
{
    ...
    public string Consumer => GameEventOutboxConsumerNames.RegionBossChat;
    ...
    {
        var payload = JsonSerializer.Deserialize<RegionBossChatAnnouncementPayload>(
            message.PayloadJson,
            jsonOptions) ?? throw new InvalidOperationException(
                "Region Boss chat announcement payload is invalid.");

        if (string.IsNullOrWhiteSpace(chatOptions.BaseUrl)
            || string.IsNullOrWhiteSpace(chatOptions.Secret))
        {
