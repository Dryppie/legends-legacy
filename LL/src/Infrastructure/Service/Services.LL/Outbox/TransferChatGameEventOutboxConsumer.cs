...
public sealed class TransferChatGameEventOutboxConsumer(
    ...
    JsonSerializerOptions jsonOptions) : IGameEventOutboxConsumer
{
    ...
    public string Consumer => GameEventOutboxConsumerNames.TransferChat;

    public bool CanHandle(string eventType) =>
        string.Equals(eventType, GameEventTypes.PlayerTransferChatMessage, StringComparison.OrdinalIgnoreCase);

    public async Task HandleAsync(
        GameEventOutboxMessage message,
        CancellationToken cancellationToken)
    {
        var payload = JsonSerializer.Deserialize<PlayerTransferChatMessagePayload>(
            message.PayloadJson,
            jsonOptions) ?? throw new InvalidOperationException("Transfer chat payload is invalid.");
