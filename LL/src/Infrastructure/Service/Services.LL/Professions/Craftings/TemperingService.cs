...
{
    public void HandleTempering(CraftingQueueItem current, Random rng)
    {
    ...
    {
        /* ---------------- probability tables ----------------
           • Critical  : 0.05 % doubled per rarity step
           • Negative  : 5 % base  +5 % per rarity step
           • Positive  : 30 % base −3 % per rarity step   (tweak this if you prefer another curve)
           • Neutral   : remainder
        ----------------------------------------------------- */

        int rarityIndex = (int)rarity;                 // Common = 0 … Legacy = 6

        double pCritical = 0.0005 * Math.Pow(2, rarityIndex);  // 0.05 % → 3.2 %
