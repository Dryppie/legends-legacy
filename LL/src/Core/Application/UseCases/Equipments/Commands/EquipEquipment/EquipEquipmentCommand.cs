...
public record EquipEquipmentCommand(Guid EntityId, string EquipmentId) : IRequest<Response<bool>>;
public class EquipEquipmentCommandHandler : IRequestHandler<EquipEquipmentCommand, Response<bool>>
{
    ...
    {
        return await _equipmentService.EquipEquipmentAsync(request.EntityId, Guid.Parse(request.EquipmentId), cancellationToken)
